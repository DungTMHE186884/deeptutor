...
from pathmind.learning import policy as learning_policy
from pathmind.learning import prompts as learning_prompts
from pathmind.learning.models import (
    KnowledgePoint,
    KnowledgeType,
    LearningModule,
    LearningProgress,
...
from pathmind.learning.service import LearningService
from pathmind.learning.storage import LearningStore
from pathmind.learning.topic_generation import MAX_MODULE_LIMIT
from pathmind.services.settings.interface_settings import get_response_language
from pathmind.utils.json_parser import parse_json_response
...
async def _cancel_active_learning_turn(book_id: str) -> None:
    from pathmind.services.session import get_turn_runtime_manager
...
async def _exclusive_path_mutation(book_id: str):
    ...
    from pathmind.learning.storage import PathLeaseConflictError
...
def _review_queue(progress, *, now: float | None = None) -> list[dict]:
    from pathmind.learning.scheduler import SpacedRepetitionScheduler, review_sort_key
...
def _read_projection(progress, *, now: float | None = None):
    ...
    from pathmind.learning.scheduler import SpacedRepetitionScheduler
...
async def generate_topic_route(body: GenerateTopicDraftRequest):
    from pathmind.learning.topic_generation import TopicGenerationError, generate_topic_draft
...
async def create_topic(body: ConfirmTopicRequest):
    from pathmind.learning.topic_generation import (
    ...
    if not resolved_name:
        from pathmind.learning.topic_naming import suggest_topic_name
...
async def set_learner_override(
    ...
    async with _exclusive_path_mutation(path_id):
        try:
            ...
            from pathmind.learning.service import MasteryInteractionError
...
async def list_topic_sessions(path_id: str):
    ...
    from pathmind.learning.navigation import topic_sessions
...
async def set_session_mode(path_id: str, session_id: str, body: SetSessionModeRequest):
    ...
    """
    from pathmind.capabilities.mastery.mode import MODES, admission_error, normalize_mode
    ...
    from pathmind.services.session import get_session_store
...
async def get_topic_ask_hint(path_id: str, session_id: str = ""):
    ...
    """
    _validate_book_id(path_id)
    from pathmind.services.mastery_hints import get_ask_hint
...
async def mastery_topic_websocket(ws: WebSocket) -> None:
    ...
    from pathmind.learning.event_hub import mastery_topic_event_hub
...
async def get_progress(book_id: str):
    ...
    if progress.pending_question is not None:
        from pathmind.learning.pending import public_pending_question
...
async def generate_from_notebook(book_id: str, body: GenerateFromNotebookRequest):
    ...
    from pathmind.services.llm import complete
...
async def generate_from_reading(book_id: str, body: GenerateFromReadingRequest):
    ...
    from pathmind.reading.knowledge_capture import mastery_source_records
    ...
    try:
        ...
        from pathmind.reading import ReadingError
