...
from pathmind.api.utils.http_headers import content_disposition
from pathmind.book import progress as progress_ops
from pathmind.book.errors import BookPausedError
from pathmind.book.export import export_filename, render_book_markdown
from pathmind.book.models import (
    BlockType,
    BookProposal,
    ContentType,
    LearningCapture,
...
from pathmind.book.storage import get_book_storage
from pathmind.book.streaming import SOURCE as BOOK_SOURCE
from pathmind.multi_user.audit import log_admin_action, log_usage
from pathmind.multi_user.book_access import (
    ResolvedBook,
    accessible_books,
    can_create_book,
    resolve_book,
...
from pathmind.multi_user.identity import remove_book_permission_overrides
...
def get_book_engine():
    ...
    from pathmind.book.engine import get_book_engine as resolve
...
def _auth_enabled() -> bool:
    from pathmind.services.auth import AUTH_ENABLED
...
async def estimate_basis(depth: str = "standard") -> dict[str, Any]:
    ...
    """
    from pathmind.book.estimate import chapter_basis
...
async def block_types() -> dict[str, list[dict[str, str | bool]]]:
    from pathmind.book.agents.page_planner import (
...
async def book_websocket(ws: WebSocket) -> None:
    ...
    **Actions** — run an engine operation and reply with a single result::
    ...
    Actions publish into the book's own stream (see :mod:`pathmind.book.event_hub`),
    ...
    from pathmind.book.event_hub import get_book_bus
    ...
    try:
        while not closed:
            ...
            try:
                ...
                from pathmind.services.workspace.context import (
