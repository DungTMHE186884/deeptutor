...
A thin adapter over :mod:`pathmind.reading`: it validates HTTP inputs, maps
...
from pathmind.reading.ingestion import (
    MAX_TRANSCRIPT_BYTES,
    ReadingIngestionService,
    url_material_id,
)
from pathmind.reading.knowledge_capture import (
    organize_workspace_notes,
    send_workspace_to_notebook,
)
from pathmind.reading.models import MAX_TEXT_SELECTOR_CHARS
from pathmind.services.session.workspace_preferences import WORKSPACE_MODE_READING
...
async def get_workspace_ask_hint(
    ...
    from pathmind.services.reading_hints import get_ask_hint
...
async def get_workspace_openers(
    ...
    """
    from pathmind.services.reading_hints import get_openers
...
async def supported_formats() -> SupportedFormats:
    ...
    from pathmind.reading.extract import RAW_VIEW_EXTENSIONS
    from pathmind.utils.document_extractor import SUPPORTED_DOC_EXTENSIONS
...
async def epub_pairing_candidates(material_id: str) -> list[dict[str, Any]]:
    from pathmind.reading.epub_bilingual import recommend_epub_candidates
...
async def epub_pairings() -> list[dict[str, Any]]:
    from pathmind.reading.epub_bilingual import list_epub_pairings
...
async def create_epub_pairing(payload: EpubPairingRequest) -> dict[str, Any]:
    from pathmind.reading.epub_bilingual import create_epub_pairing
...
async def remove_epub_pairing(pairing_id: str) -> dict[str, Any]:
    from pathmind.reading.epub_bilingual import delete_epub_pairing
...
async def get_snapshot_asset(material_id: str, asset_name: str) -> FileResponse:
    ...
    from pathmind.services.web_source.snapshot_assets import snapshot_asset_mime
