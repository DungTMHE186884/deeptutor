...
from pathmind.app.container import get_application_container
from pathmind.multi_user.context import get_current_user
from pathmind.multi_user.partner_access import (
...
from pathmind.partners.config.paths import get_partner_media_dir
from pathmind.partners.helpers import safe_filename
from pathmind.runtime.coordination import BackgroundCommandKind
from pathmind.services.i18n import t
from pathmind.services.partners import (
    get_partner_manager,
    slugify_partner_id,
    slugify_soul_id,
)
from pathmind.services.partners.channel_onboarding import (
    ChannelOnboardingError,
    get_channel_onboarding_manager,
)
from pathmind.services.partners.drafts import PartnerDraftStore
from pathmind.services.partners.manager import (
...
from pathmind.services.partners.runtime_status import (
    get_partner_runtime_status_repository,
)
from pathmind.services.partners.workspace import (
    list_assets,
    provision_assets,
    read_soul,
    remove_asset,
...
def _validate_channels_payload(channels: dict) -> None:
    ...
    """
    from pathmind.partners.config.schema import ChannelsConfig
    ...
    from pathmind.api.routers._partners_channel_schema import resolve_config_model
    from pathmind.partners.channels.registry import discover_all
...
def _validate_llm_selection_payload(
    ...
    from pathmind.multi_user.model_access import apply_allowed_llm_selection
    ...
    from pathmind.services.partners.model_runtime import normalize_partner_llm_selection
...
def _caller_tool_reach() -> tuple[set[str] | None, set[str] | None]:
    ...
    from pathmind.multi_user.tool_access import allowed_mcp_tools, allowed_optional_tools
...
def _resolve_soul_content(soul: SoulSpec | None) -> tuple[str, dict[str, str]]:
    ...
    from pathmind.services.partners.workspace import DEFAULT_SOUL
...
def _load_persona_markdown(name: str) -> str:
    from pathmind.multi_user.paths import get_admin_path_service
    from pathmind.services.persona import PersonaService, get_persona_service
...
async def get_partner_consultation_session(chat_session_id: str, partner_name: str):
    """Recover native identity for older consultation traces."""
    return None
...
async def list_channel_schemas():
    ...
    from pathmind.api.routers._partners_channel_schema import all_channel_schemas
...
async def start_weixin_qr(partner_id: str):
    ...
    from pathmind.services.partners import weixin_onboarding
...
async def tool_options():
    ...
    """
    from pathmind.agents._shared.tool_composition import admin_enabled_optional_tools
    from pathmind.api.utils.tool_options import build_tool_options
    from pathmind.multi_user.tool_access import combine_whitelists
...
def _validate_workspace(workspace_id: str, owner_id: str) -> str:
    from pathmind.services.partners.workspace_binding import validate_partner_workspace
    from pathmind.services.workspace import WorkspaceError
...
async def start_partner(partner_id: str):
    ...
    # comes back in this state after a PathMind restart (a manual /stop clears
...
async def get_partner_channel_status(partner_id: str):
    ...
    from pathmind.services.partners.channel_onboarding import _qr_data_url
...
async def get_partner_workspaces(partner_id: str):
    ...
    from pathmind.multi_user.paths import user_context
    from pathmind.services.partners.workspace_binding import workspace_owner
...
async def create_partner_link_code(partner_id: str):
    ...
    """
    from pathmind.services.partners.links import issue_link_code
...
async def list_partner_links(partner_id: str):
    ...
    from pathmind.services.partners.links import list_links
...
async def delete_partner_link(partner_id: str, key: str):
    ...
    from pathmind.services.partners.links import remove_link
...
async def partner_command_palette():
    from pathmind.services.partners.commands import partner_command_palette
...
async def _partner_chat_stream(
    ...
    from pathmind.core.stream import StreamEventType
...
async def partner_chat_ws(ws: WebSocket, partner_id: str):
    ...
    * ``{"type": "done"}`` / ``{"type": "error"}`` / ``{"type": "proactive"}``.
    ...
    from pathmind.multi_user.context import get_current_user_or_none, reset_current_user
    from pathmind.services.partners.interaction import personal_actor_id
