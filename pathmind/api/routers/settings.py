...
from pathmind.multi_user.model_access import allowed_llm_options
from pathmind.services.codebuddy_auth import get_codebuddy_auth_service
from pathmind.services.codex_auth import (
    CodexAuthError,
    get_codex_oauth_service,
    reconcile_codex_catalog_update,
)
from pathmind.services.config import (
    CATALOG_SECRET_MASK,
...
from pathmind.services.config.origins import normalize_origins
from pathmind.services.config.runtime_settings import (
    CHAT_ATTACHMENT_CHARS_RANGE,
    CHAT_ATTACHMENT_MAX_FILE_MB_RANGE,
    CHAT_ATTACHMENT_MAX_TOTAL_MB_RANGE,
    compute_ws_max_size,
)
from pathmind.services.config.settings_draft import (
    get_settings_draft_service,
    is_empty_draft,
    merge_draft_secrets,
    redact_draft,
)
from pathmind.services.llm.config import clear_llm_config_cache
...
from pathmind.services.settings.interface_settings import (
...
from pathmind.services.settings.starter_settings import (
    TRACE_COUNT_RANGE as STARTER_TRACE_COUNT_RANGE,
)
from pathmind.tools.builtin import USER_TOGGLEABLE_TOOL_NAMES
...
def _runtime_catalog_write() -> Iterator[None]:
    ...
    from pathmind.services.embedding.client import reset_embedding_client
    from pathmind.services.llm.client import reset_llm_client
...
def _require_codex_oauth_actor() -> None:
    ...
    """
    from pathmind.services.partners.scope import is_partner_user_id
...
def _provider_choices() -> dict[str, list[dict[str, Any]]]:
    ...
    from pathmind.services.config.provider_runtime import (
    ...
    from pathmind.services.provider_registry import PROVIDERS
    ...
    from pathmind.services.voice.options import voice_options
...
async def get_settings():
    ...
    from pathmind.services.model_selection.tasks import task_kind_payload
    ...
    return {
        ...
        # cannot list a task PathMind no longer makes — or miss a new one.
...
def _chat_attachments_payload() -> dict[str, Any]:
    """Attachment caps for the caller.

    The system-wide caps in ``system.json`` are the ceiling; the per-file cap
    is further limited by the caller's subscription plan
    (``max_upload_file_size_bytes``) — see ``get_chat_attachment_limits``.
    """
    from pathmind.services.config.runtime_settings import get_chat_attachment_limits
...
async def get_chat_attachment_settings():
    """Chat attachment caps for the caller (read-only).

    Caps are managed per subscription plan in /admin/plans; the old admin
    "Attachments" settings page and its PUT endpoint were removed.
    """
...
def _mineru_settings_payload() -> dict[str, Any]:
    ...
    """
    from pathmind.services.parsing.engines.mineru.backend import local_cli_probe
...
def _document_parsing_payload() -> dict[str, Any]:
    ...
    from pathmind.services.parsing.engines._install import (
        installable_engines,
        model_downloadable_engines,
    )
    from pathmind.services.parsing.engines.factory import (
        get_parser,
        list_engines,
    )
    from pathmind.services.parsing.engines.mineru.backend import local_cli_probe
...
async def get_settings_readiness():
    ...
    _require_settings_admin()
    from pathmind.services.config.readiness import build_settings_readiness
...
async def test_document_parsing(payload: DocumentParsingTest):
    ...
    _require_settings_admin()
    from pathmind.services.parsing.engines.factory import get_parser, is_engine_available
...
async def test_docling_remote_connection(payload: DoclingRemoteTest):
    ...
    is untouched."""
    _require_settings_admin()
    from pathmind.services.parsing.engines.docling.config import (
        DoclingConfig,
        resolve_docling_config,
    )
    from pathmind.services.parsing.engines.docling.remote import verify_remote
...
async def test_tika_remote_connection(payload: TikaRemoteTest):
    ...
    _require_settings_admin()
    from pathmind.services.parsing.engines.tika.config import TikaConfig
    from pathmind.services.parsing.engines.tika.remote import verify_remote
    ...
    if not cmd:
        return {
            ...
            "message": (
                ...
                f"(pip install pathmind[parse-{engine}]) so its CLI is on PATH."
            ),
        }
...
async def document_parsing_job_status(cursor: int = 0):
    _require_settings_admin()
    from pathmind.services.parsing.engines._install import get_background_job_manager
...
async def start_mineru_models_download(payload: MinerUModelDownloadPayload):
    ...
    """
    _require_settings_admin()
    from pathmind.services.parsing.engines.mineru.models import (
...
async def mineru_models_download_status(cursor: int = 0):
    _require_settings_admin()
    from pathmind.services.parsing.engines.mineru.models import get_model_download_manager
...
async def test_mineru_connection(payload: MinerUSettingsUpdate):
    ...
    _require_settings_admin()
    from pathmind.services.parsing.engines.mineru.cloud import verify_credentials
    from pathmind.services.parsing.engines.mineru.config import MinerUConfig, MinerUError

    if payload.mode == "local":
        from pathmind.services.parsing.engines.mineru.backend import (
            local_cli_probe,
            local_cli_version,
        )
        from pathmind.services.parsing.engines.mineru.formats import (
            MIN_MINERU_VERSION,
        ...
        if not mineru_version_is_current(version):
            ...
            return {
                "ok": False,
                "message": (
                    f"Local MinerU CLI reported {detail}. PathMind needs MinerU >= "
...
async def apply_registry_edit(payload: RegistryEditPayload):
    ...
    _require_settings_admin()
    from pathmind.services.settings.registry_edit import merge_registry_edit
...
async def apply_provider_edit(payload: ProviderEditPayload):
    ...
    _require_settings_admin()
    from pathmind.services.settings.provider_edit import merge_provider_edit
...
async def apply_catalog_service(payload: CatalogServicePayload):
    ...
    if payload.service == "task" and payload.config.get("mode") == "reference":
        from pathmind.services.model_selection import apply_llm_selection_to_catalog
...
async def test_provider_connection(payload: ProviderProbePayload):
    ...
    _require_settings_admin()
    from pathmind.services.settings.provider_probe import probe_provider, probe_search_provider
    ...
    from pathmind.services.keypool import primary_api_key
...
async def fetch_models_from_provider(payload: FetchModelsPayload):
    ...
    """
    _require_settings_admin()
    from pathmind.services.llm.factory import fetch_models as fetch_llm_models
...
async def resolve_model_capabilities(payload: ModelCapabilitiesQuery):
    ...
    """
    _require_settings_admin()
    from pathmind.services.llm.capabilities import effective_capabilities
...
def _optional_auth_dependency():
    from pathmind.api.routers.auth import optional_auth

    return optional_auth


@public_router.get("/ui")
async def get_ui_settings(_user=Depends(_optional_auth_dependency())):
    ...
    A signed-in caller gets **their own** saved preferences (the session
    cookie is honoured but not required), so theme/language follow the
    account across browsers. Anonymous visitors get the deployment default
    theme (``PATHMIND_DEFAULT_THEME``, "snow" if unset).
    """
    settings = load_ui_settings()
    result = {field: settings.get(field) for field in PRESESSION_UI_FIELDS}
    from pathmind.services.auth import AUTH_ENABLED as _auth_enabled

    if _auth_enabled and _user is None:
        # Nobody is signed in: never show the administrator's personal theme
        # on the login/register pages — use the deployment default instead.
        from pathmind.services.settings.interface_settings import DEFAULT_THEME
...
async def preview_voice(payload: VoicePreviewPayload) -> Response:
    ...
    _require_settings_admin()
    from pathmind.services.voice.base import VoiceProviderError
    from pathmind.services.voice.preview import synthesize_preview
...
async def complete_tour(payload: TourCompletePayload | None = None):
    ...
    return {
        "status": "completed",
        "message": "Configuration saved. PathMind will restart shortly.",
        "launch_at": launch_at,
