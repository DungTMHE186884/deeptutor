...
come from :mod:`pathmind.services.suggestions`, which reads memory.
...
async def get_starter_suggestions():
    ...
    """
    from pathmind.services.suggestions import get_suggestions
...
async def refresh_starter_suggestions():
    ...
    different set.
    """
    from pathmind.services.suggestions import refresh_suggestions
...
async def get_learning_index():
    ...
    from pathmind.api.routers.book import list_books
    from pathmind.api.routers.mastery_path import list_topics
    from pathmind.api.routers.reading import list_workspaces
    from pathmind.services.workspace.navigation import read_workspace_indexes
    ...
    async def watching():
        from pathmind.services.session.organization import list_all_sessions_snapshot
...
async def get_source_library(kind: str):
    ...
    async def read():
        if kind == "notebooks":
            from pathmind.services.notebook import notebook_manager
        ...
        if kind == "drafts":
            from pathmind.api.routers.co_writer import list_documents

            return [row.model_dump() for row in (await list_documents())["documents"]]
        raise HTTPException(404, "Unknown source library")

    if kind == "knowledge":
        from pathmind.services.workspace.knowledge import knowledge_catalog
...
async def get_learning_library(kind: str):
    ...
    async def read():
        ...
        if kind == "reading":
            ...
            return (await list_workspaces(search=""))["workspaces"]
        if kind == "materials":
            from pathmind.api.routers.reading import list_library_materials
