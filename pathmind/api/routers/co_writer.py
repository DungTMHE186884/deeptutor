...
from pathmind.co_writer.docx_converter import (
    DocxConversionError,
    docx_to_markdown,
    markdown_to_docx,
)
from pathmind.co_writer.storage import (
    CoWriterDocument,
...
if TYPE_CHECKING:
    from pathmind.co_writer.edit_agent import EditAgent
...
def _clean_react_edit_output(text: str, *, binding: str | None, model: str | None) -> str:
    from pathmind.services.llm import clean_thinking_tags
...
async def _run_react_edit(
    ...
    *,
    ...
    from pathmind.co_writer.edit_agent import append_history, print_stats
    ...
    async def _consume() -> None:
        if pageindex_source and request.kb_name:
            from pathmind.services.rag.pipelines.pageindex.reasoning import (
...
async def edit_text(request: EditRequest):
    from pathmind.co_writer.edit_agent import print_stats
...
async def get_history():
    ...
    from pathmind.co_writer.edit_agent import load_history
...
async def get_tool_call(operation_id: str):
    ...
    from pathmind.co_writer.edit_agent import tool_calls_dir
