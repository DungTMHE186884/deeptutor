...
to pair with this PathMind instance and push synced study data.

Authentication layers:
* ``/pair``, ``/devices``, ``/status`` -- PathMind session auth (the logged-in
...
from pathmind.capabilities.marginnote4.models import MarginNoteObject, SyncBatch
from pathmind.capabilities.marginnote4.store import MarginNoteStore, resolve_db_path
...
# -- session-authenticated endpoints (PathMind user) -----------------------
...
async def pair_device(body: PairRequest, request: Request) -> PairResponse:
    """Pair a new MN4 device. Requires a PathMind session.
