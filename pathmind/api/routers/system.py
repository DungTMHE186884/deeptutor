...
from pathmind.api.contracts.turn_protocol import (
...
from pathmind.runtime import memory_probe
...
def get_embedding_client(*args, **kwargs):
    from pathmind.services.embedding.client import get_embedding_client as resolve

    return resolve(*args, **kwargs)


def get_embedding_config(*args, **kwargs):
    from pathmind.services.embedding.config import get_embedding_config as resolve

    return resolve(*args, **kwargs)


def get_llm_config(*args, **kwargs):
    from pathmind.services.llm.config import get_llm_config as resolve

    return resolve(*args, **kwargs)


def get_token_limit_kwargs(*args, **kwargs):
    from pathmind.services.llm.config import get_token_limit_kwargs as resolve
...
def web_search(*args, **kwargs):
    from pathmind.services.search import web_search as search
...
async def request_managed_update(_request: ManagedUpdateRequest) -> dict[str, Any]:
    ...
    if not launcher_available():
        raise HTTPException(
            status_code=409,
            detail="Web updates require PathMind to be running under `pathmind start`.",
        )
    installation = get_update_installation()
    ...
    if not result.update_available:
        raise HTTPException(status_code=409, detail="No newer PathMind release is available")
    ...
    if job is None:
        raise HTTPException(
            status_code=409,
            detail="Finish the active conversation before updating PathMind.",
        )
...
async def get_memory_usage():
    """Resident memory of the running PathMind process tree.
...
async def test_search_connection():
    ...
    try:
        ...
        result = web_search("PathMind health check", provider=search_config.provider)
        response_time = (time.time() - start_time) * 1000
