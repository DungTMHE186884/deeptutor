...
from pathmind.api.utils.progress_broadcaster import ProgressBroadcaster
from pathmind.api.utils.task_id_manager import TaskIDManager
from pathmind.api.utils.task_log_stream import capture_task_logs, get_task_stream_manager
from pathmind.knowledge.add_documents import DocumentAdder, remove_raw_document
from pathmind.knowledge.initializer import KnowledgeBaseInitializer
from pathmind.knowledge.kb_types import is_connected_kb, supports_local_raw_files
from pathmind.knowledge.manager import KnowledgeBaseManager
from pathmind.knowledge.naming import validate_knowledge_base_name
from pathmind.knowledge.progress_tracker import ProgressStage, ProgressTracker
from pathmind.logging import PROCESS_LOG_PRIVATE_ATTR
...
from pathmind.multi_user.knowledge_access import (
    assert_writable,
    current_kb_base_dir,
    current_kb_manager,
...
from pathmind.services.config import PROJECT_ROOT, load_config_with_main
from pathmind.services.file_io import atomic_write_json
from pathmind.services.rag.factory import (
    DEFAULT_PROVIDER,
    GRAPHRAG_PROVIDER,
    LIGHTRAG_PROVIDER,
...
from pathmind.services.rag.file_routing import FileTypeRouter
from pathmind.services.rag.linked_kb import (
    LINKABLE_PROVIDERS,
    assert_path_allowed,
    probe_linked_folder,
)
from pathmind.services.rag.pipelines.ima.client import (
    MAX_PAGE_LIMIT,
    ImaAPIError,
    ImaAuthError,
    ImaClient,
...
from pathmind.services.rag.pipelines.ima.config import (
    ImaConfig,
    ImaCredentials,
    get_account_credentials,
)
from pathmind.utils.document_extractor import (
    MAX_EXTRACTED_CHARS_PER_DOC,
    DocumentExtractionError,
    extract_text_from_path,
    extract_text_from_path_isolated,
)
from pathmind.utils.document_validator import DocumentValidator
from pathmind.utils.error_utils import format_exception_message
...
def _save_zip_archive(
    ...
    from pathmind.utils.archive_extractor import ArchiveTooLargeError, safe_extract_zip
...
def _save_uploaded_files(
    ...
    from pathmind.services.pocketbase_client import is_pocketbase_enabled
...
def _upload_file_to_pb(kb_name: str, filename: str, file_path: Path) -> None:
    ...
    try:
        from pathmind.services.pocketbase_client import get_pb_client
...
def _freeze_indexing_llm_form(raw: str):
    ...
    from pathmind.services.rag.pipelines.lightrag.indexing_policy import (
        IndexingPolicyError,
        freeze_snapshot,
    )
...
def _assert_provider_ready(provider: str) -> None:
    ...
    if provider == PAGEINDEX_PROVIDER:
        from pathmind.services.rag.pipelines.pageindex.config import is_pageindex_configured
    ...
    if provider == PAGEINDEX_OSS_PROVIDER:
        from pathmind.services.rag.preflight import engine_preflight
    ...
    if provider == GRAPHRAG_PROVIDER:
        from pathmind.services.rag.pipelines.graphrag.config import is_graphrag_available
        ...
        if not is_graphrag_available():
            raise HTTPException(
                ...
                detail=(
                    ...
                    "`pip install 'pathmind[graphrag]'` on the server before "
    ...
    if provider == LIGHTRAG_PROVIDER:
        from pathmind.services.rag.pipelines.lightrag.config import is_lightrag_available
        ...
        if not is_lightrag_available():
            raise HTTPException(
                ...
                detail=(
                    ...
                    "`pip install 'pathmind[rag-lightrag]'` on the server before "
...
def _resolve_embedding_form(value, provider: str, entry: dict | None = None):
    from pathmind.services.embedding.config import get_embedding_config
    from pathmind.services.rag.embedding_binding import EMBEDDING_PROVIDERS, default_selection
...
def _enforce_provider_formats(provider: str, files: list[UploadFile]) -> None:
    ...
    if provider not in {PAGEINDEX_PROVIDER, PAGEINDEX_OSS_PROVIDER}:
        return
    from pathmind.services.rag.pipelines.pageindex.pipeline import (
...
def _matching_index_is_valid(kb_name: str, matching_version: dict | None) -> bool:
    ...
    if not matching_version:
        return False
    try:
        from pathmind.services.rag.index_probe import inspect_provider_version
        from pathmind.services.rag.pipelines.llamaindex.storage import (
...
async def get_rag_providers():
    ...
    try:
        from pathmind.services.config import get_kb_config_service
        from pathmind.services.rag.service import RAGService
...
def _pageindex_config_payload() -> dict:
    ...
    from pathmind.services.config import get_runtime_settings_service
...
def _validate_lightrag_llm_selection(profile_id: str, model_id: str) -> None:
    ...
    if not profile_id:
        return
    try:
        from pathmind.services.config import get_model_catalog_service
        from pathmind.services.model_selection import (
            LLMSelection,
...
async def update_lightrag_server_pipeline_config(payload: LightRagServerConfigUpdate):
    ...
    try:
        from pathmind.services.config import get_runtime_settings_service
        from pathmind.services.rag.pipelines.lightrag_server.config import normalize_base_url
...
async def get_embedding_usage():
    ...
    from pathmind.multi_user.paths import get_account_path_service
    from pathmind.services.config.knowledge_base_config import KnowledgeBaseConfigService
    from pathmind.services.path_service import get_path_service
    from pathmind.services.rag.embedding_binding import (
        load_catalog,
        migrate_binding,
...
async def _probe_graphrag_model_compatibility(profile_id: str, model_id: str) -> dict:
    ...
    from pathmind.services.rag.pipelines.graphrag.compatibility import (
...
async def update_kb_config(kb_name: str, config: dict):
    ...
    try:
        from pathmind.services.config import get_kb_config_service
        from pathmind.services.rag.index_probe import has_ready_provider_index
...
async def probe_lightrag_server_route(payload: ProbeLightRagServerRequest):
    ...
    """
    from pathmind.services.rag.pipelines.lightrag_server.probe import probe_server
    ...
    if payload.use_saved_api_key and not api_key:
        from pathmind.services.config import load_lightrag_server_settings
        from pathmind.services.rag.pipelines.lightrag_server.config import normalize_base_url
...
async def connect_lightrag_server_route(payload: ConnectLightRagServerRequest):
    ...
    """
    from pathmind.services.rag.pipelines.lightrag_server.config import SUPPORTED_MODES
    from pathmind.services.rag.pipelines.lightrag_server.probe import probe_server
...
async def probe_weknora_route(payload: ProbeWeKnoraRequest):
    ...
    from pathmind.services.rag.pipelines.weknora.probe import probe_weknora
...
async def probe_ima_route(payload: ProbeImaRequest):
    ...
    Creates nothing.
    """
    from pathmind.services.rag.pipelines.ima.probe import probe_knowledge_base
...
async def list_knowledge_bases():
    ...
    from pathmind.services.workspace.resources import current_resources
...
def _resolve_kb_raw_dir(kb_name: str, *, allow_unsupported: bool = False) -> Path | None:
    ...
    PathMind-managed raw directory.  File listing treats that as an empty
...
async def delete_kb_file(kb_name: str, filename: str):
    ...
    if provider in {PAGEINDEX_PROVIDER, PAGEINDEX_OSS_PROVIDER}:
        from pathmind.services.rag.factory import get_pipeline
    ...
    try:
        released = target.stat().st_size if target.is_file() else 0
    except OSError:
        released = 0
    removal = remove_raw_document(Path(kb_dir), target)
    _release_storage(released)
    return {
        "status": "ok",
...
def _release_storage(released_bytes: int) -> None:
    """Give storage quota back to the current user after deleting files."""
    if released_bytes <= 0:
        return
    try:
        from pathmind.database.connection import get_db_session
        from pathmind.services.quota_service import update_user_storage

        with get_db_session() as db:
            update_user_storage(get_current_user().id, -int(released_bytes), db)
    except Exception as exc:
        logger.warning("Storage release failed: %s", exc)
...
def _delete_kb(kb_name: str) -> dict[str, str]:
    ...
    try:
        ...
        try:
            released = _dir_size(Path(manager.get_knowledge_base_path(resolved_name)) / "raw")
        except Exception:
            released = 0
...
async def upload_files(
    ...
    try:
        ...
        from pathmind.services.embedding.config import embedding_config_scope
        from pathmind.services.rag.embedding_binding import binding_status
        ...
        # Storage Quota Enforcement for current user
        current_user = get_current_user()
        try:
            from pathmind.database.connection import get_db_session
            from pathmind.services.quota_service import check_storage_batch, update_user_storage

            with get_db_session() as db:
                sizes = [_get_upload_file_size(f) or 0 for f in files]
                allowed, reason = check_storage_batch(
                    current_user.id, sizes, db, role=current_user.role
                )
                if not allowed:
            ...
            logger.warning("Storage quota check skipped: %s", q_exc)
        ...
        try:
            total_added = sum(os.path.getsize(p) for p in uploaded_file_paths if os.path.exists(p))
            with get_db_session() as db:
                update_user_storage(current_user.id, total_added, db)
        except Exception as q_exc:
            logger.warning("Storage usage update failed: %s", q_exc)
    ...
    try:
        ...
        if name in manager.config.get("knowledge_bases", {}):
            ...
            if embedding_selection:
                from pathmind.services.rag.embedding_binding import binding_fields
...
async def run_reindex_task(
    ...
    with capture_task_logs(task_id):
        try:
            ...
            # provider=None → RAGService resolves the KB's PathMind-bound
...
async def reindex_knowledge_base(
    ...
    try:
        ...
        if provider_uses_embedding_versions(kb_provider):
            from pathmind.services.rag.embedding_signature import (
                signature_from_config,
                signature_from_embedding_config,
            )
            from pathmind.services.rag.index_versioning import (
...
async def update_pending_indexing_policy(
    ...
    kb_dir = kb_base_dir / kb_name
    from pathmind.services.rag.pipelines.lightrag.storage import latest_published_root
...
async def websocket_progress(websocket: WebSocket, kb_name: str):
    ...
    from pathmind.api.routers.auth import ws_auth_failed, ws_require_auth
    from pathmind.multi_user.context import reset_current_user
    ...
    try:
        from pathmind.services.workspace.knowledge import parse_kb_id
...
async def sync_github_sources(kb_name: str):
    with _knowledge_source_errors(kb_name):
        ...
        from pathmind.services.github_source.sync import sync_source
...
async def sync_web_sources(kb_name: str):
    ...
    with _knowledge_source_errors(kb_name):
        ...
        from pathmind.services.web_source.sync import sync_source
