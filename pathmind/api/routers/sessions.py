...
from pathmind.services.session import get_session_store, get_sqlite_session_store
...
from pathmind.services.session.search import MAX_SEARCH_QUERY_CHARS
from pathmind.services.storage.attachment_store import get_attachment_store
...
async def list_sessions(
    ...
    if all_workspaces:
        from pathmind.services.workspace.navigation import session_index
...
async def get_session_ask_hint(session_id: str) -> dict[str, Any]:
    ...
    from pathmind.services.chat_hints import get_ask_hint
...
async def update_session_organization(
    ...
    if "workspace_id" in fields:
        ...
        if current_scope is not None and current_scope.workspace_id != workspace_id:
            ...
            from pathmind.api.routers.workspace import _data_operation
            from pathmind.services.workspace.session_move import move_chat
    ...
    if "course_id" in fields:
        ...
        if course_id:
            from pathmind.services.courses import CourseNotFoundError, get_course_service
