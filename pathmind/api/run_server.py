...
def main() -> None:
    ...
    from pathmind.runtime.mode import RunMode, set_mode
    from pathmind.services.config import (
        HTTP_KEEP_ALIVE_TIMEOUT,
        get_ws_max_size,
        load_system_settings,
    )
    from pathmind.services.setup import get_backend_port
    ...
    dev_reload = os.environ.get("PATHMIND_DEV_RELOAD", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    if dev_reload and backend_workers > 1:
        raise SystemExit(
            ...
            "Set backend_workers=1 or disable PATHMIND_DEV_RELOAD."
        )
    uvicorn.run(
