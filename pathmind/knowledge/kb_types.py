...
Kept in its own low-level module so both :mod:`pathmind.knowledge.manager`
...
# indexing and document management; PathMind only stores retrieval credentials.
...
NON_RETRIEVABLE_KB_TYPES = frozenset({OBSIDIAN_KB_TYPE, MARGINNOTE4_KB_TYPE})
...
def supports_local_raw_files(entry: Any) -> bool:
    """Whether the KB owns a PathMind-managed local ``raw/`` directory.
    ...
    PathMind's raw-file upload and management API.
    """
