...
from pathmind.knowledge.kb_types import (
    IMA_KB_TYPE,
...
from pathmind.knowledge.manifest import iter_kb_documents
...
from pathmind.services.rag.index_probe import (
    inspect_kb_versions,
    inspect_provider_version,
    provider_failure_summary,
)
from pathmind.services.web_source.crawler import MAX_CRAWL_DEPTH, MAX_CRAWL_PAGES
...
def _get_embedding_fingerprint() -> tuple[str, int] | None:
    ...
    try:
        from pathmind.services.embedding import get_embedding_config
...
def _reconcile_embedding_flags(knowledge_bases: dict, base_dir: Path | None = None) -> bool:
    ...
    * No match, but the KB has a stored ``embedding_model`` that differs
    ...
    """
    from pathmind.services.rag.embedding_signature import signature_from_embedding_config
    ...
    if base_dir is not None:
        from pathmind.services.rag.embedding_binding import reconcile_bindings
...
class KnowledgeBaseManager:
    ...
    def update_kb_status(
        ...
        if status == "ready":
            ...
            try:
                from pathmind.services.rag.embedding_binding import entry_signature
    ...
    def list_knowledge_bases(self) -> list[str]:
        ...
        if base_exists:
            for item in self.base_dir.iterdir():
                ...
                from pathmind.services.rag.index_versioning import list_kb_versions
    ...
    def _marginnote4_store_owner(
        ...
        from pathmind.capabilities.marginnote4.store import resolve_db_path
    ...
    def get_rag_storage_path(self, name: str | None = None) -> Path:
        ...
        from pathmind.services.rag.embedding_binding import binding_status, entry_signature
        from pathmind.services.rag.index_versioning import (
    ...
    def get_info(
        ...
        *,
        ...
        if rag_provider == LIGHTRAG_PROVIDER:
            from pathmind.services.rag.pipelines.lightrag.storage import (
        ...
        if dir_exists:
            try:
                ...
                # counts for the same KB (see :mod:`pathmind.knowledge.manifest`).
