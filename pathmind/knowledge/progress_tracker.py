...
class ProgressTracker:
    ...
    def _notify(self, progress: dict):
        ...
        from pathmind.runtime.mode import is_server

        if is_server():
            try:
                from pathmind.knowledge.progress_events import broadcast_progress
    ...
    def update(
        ...
        try:
            ...
            fallback_logger = logging.getLogger("pathmind.ProgressTracker")
            prefix = f"[{self.task_id}]" if self.task_id else ""
        ...
        if self.task_id:
            try:
                from pathmind.knowledge.progress_events import emit_task_progress
