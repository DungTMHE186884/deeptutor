...
from pathmind.learning.models import TopicSource, TopicSourceKind
from pathmind.learning.topic_materials import (
...
def two_chapter_book(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("pathmind.book.storage.get_book_storage", lambda: _book_storage(chapters))
    monkeypatch.setattr(
        "pathmind.book.context.build_book_context",
...
def test_one_failing_material_never_takes_the_turn_down(
    ...
    monkeypatch.setattr("pathmind.services.notebook.get_notebook_manager", _explode)
...
def test_notebook_becomes_one_source_carrying_its_records(
    ...
    monkeypatch.setattr(
        "pathmind.services.notebook.get_notebook_manager",
...
def test_a_chat_transcript_becomes_readable_material(monkeypatch):
    ...
    monkeypatch.setattr(
        "pathmind.services.session.source_inventory._load_history_session", _fake_load
    )
    monkeypatch.setattr(
        "pathmind.learning.topic_materials._session_store", lambda: SimpleNamespace()
    )
...
def test_a_question_bank_entry_becomes_readable_material(monkeypatch):
    ...
    monkeypatch.setattr(
        "pathmind.services.session.source_inventory._load_question_entry", _fake_entry
    )
    monkeypatch.setattr(
        "pathmind.learning.topic_materials._session_store", lambda: SimpleNamespace()
    )
...
def test_a_cowriter_draft_becomes_readable_material(monkeypatch):
    monkeypatch.setattr(
        "pathmind.co_writer.storage.get_co_writer_storage",
...
def test_a_partner_group_conversation_becomes_readable_material(monkeypatch):
    monkeypatch.setattr(
        "pathmind.services.session.source_inventory._load_partner_group_reference",
        lambda ref, language: (
