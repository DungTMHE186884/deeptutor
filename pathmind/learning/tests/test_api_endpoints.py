...
from pathmind.api.routers.mastery_path import router, ws_router
from pathmind.learning.models import LearningProgress, PendingQuestion, QuizAttempt
...
def app(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "pathmind.api.routers.mastery_path.LearningStore",
...
class TestListProgress:
    def test_list_runs_blocking_store_work_in_worker_thread(self, client):
        with patch(
            "pathmind.api.routers.mastery_path.asyncio.to_thread",
...
class TestTopicProductApi:
    ...
    def test_topic_sessions_include_message_count_and_latest_preview(self, client, monkeypatch):
        ...
        monkeypatch.setattr(
            "pathmind.services.session.get_session_store",
    ...
    def test_generate_mixed_source_draft_without_persisting(self, client):
        ...
        with patch(
            "pathmind.learning.topic_generation.complete",
...
class TestGenerateFromNotebook:
    ...
    @patch("pathmind.services.llm.complete", new_callable=AsyncMock)
    ...
    @patch("pathmind.api.routers.mastery_path.get_response_language", return_value="en")
    @patch("pathmind.services.llm.complete", new_callable=AsyncMock)
    ...
    @patch("pathmind.api.routers.mastery_path.get_response_language", return_value="zh")
    @patch("pathmind.services.llm.complete", new_callable=AsyncMock)
