...
from pathmind.knowledge.manifest import KbDocument, KbManifest
...
async def test_the_libraries_file_list_reaches_the_prompt(
    ...
    monkeypatch.setattr(
        "pathmind.multi_user.knowledge_access.resolve_kb_manifest",
        lambda *args, **kwargs: manifest,
    )
    monkeypatch.setattr(
        "pathmind.tools.rag_tool.rag_search",
        AsyncMock(
    ...
    monkeypatch.setattr("pathmind.learning.topic_generation.complete", complete)
...
async def test_a_single_selected_file_is_read_rather_than_retrieved(
    ...
    monkeypatch.setattr(
        "pathmind.multi_user.knowledge_access.resolve_kb_document_path",
        lambda kb, rel: lesson,
    )
    rag_search = AsyncMock()
    monkeypatch.setattr("pathmind.tools.rag_tool.rag_search", rag_search)
    complete = AsyncMock(
