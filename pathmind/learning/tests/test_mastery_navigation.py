...
The navigation layer (:mod:`pathmind.learning.navigation`) and the four
permanently-mounted tools (:mod:`pathmind.tools.mastery_nav`) answer "what am
...
from pathmind.learning import navigation
...
from pathmind.services.session.sqlite_store import SQLiteSessionStore
from pathmind.tools.mastery_nav import (
...
def session_store(tmp_path, monkeypatch) -> SQLiteSessionStore:
    ...
    monkeypatch.setattr("pathmind.services.session.get_session_store", lambda: store)
    monkeypatch.setattr("pathmind.services.session.get_sqlite_session_store", lambda: store)
    return store
