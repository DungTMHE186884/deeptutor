...
:class:`~pathmind.capabilities.explore_context.ExploreContextCapability`'s
...
:class:`~pathmind.capabilities.mastery.loop.MasteryLoopCapability`) are wired
up in :mod:`pathmind.services.session.turn_runtime`.
...
def _load_chat_material(source_id: str, label: str, budget: int) -> TopicMaterial:
    ...
    """
    from pathmind.services.session.source_inventory import _load_history_session
...
def _load_question_bank_material(source_id: str, label: str, budget: int) -> TopicMaterial:
    ...
    """
    from pathmind.services.session.source_inventory import _load_question_entry
...
def _load_cowriter_material(source_id: str, label: str, budget: int) -> TopicMaterial:
    ...
    from pathmind.co_writer.storage import get_co_writer_storage
...
def _load_partner_group_material(source_id: str, label: str, budget: int) -> TopicMaterial:
    ...
    from pathmind.services.session.source_inventory import _load_partner_group_reference
