...
from pathmind.core.capability_protocol import CapabilityManifest, TurnCapability
from pathmind.core.context import UnifiedContext
from pathmind.core.trace import merge_trace_metadata
from pathmind.i18n import StatusI18n
from pathmind.runtime.agentic.usage import UsageTracker
from pathmind.runtime.request_contracts import (
...
class VisualizeCapability(TurnCapability):
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        ...
        from pathmind.agents.chat.agentic_pipeline import AgenticChatPipeline
    ...
    def _tool_calling_disabled_for() -> str:
        ...
        """
        try:
            from pathmind.runtime.agentic.client import can_use_native_tool_calling
            from pathmind.services.llm.config import get_llm_config
        ...
        *,
        ...
        if importlib.util.find_spec("manim") is None:
            raise RuntimeError(
                ...
                "Install with `pip install 'pathmind[math-animator]'` "
                "or `pip install -r requirements/math-animator.txt`."
            )

        from pathmind.agents.math_animator.pipeline import MathAnimatorPipeline
        from pathmind.agents.math_animator.request_config import MathAnimatorRequestConfig
        from pathmind.core.trace import build_trace_metadata, new_call_id
        from pathmind.services.llm.config import get_llm_config
