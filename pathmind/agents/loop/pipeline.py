...
This is the loop's *host*: it owns everything :class:`~pathmind.agents.loop.agent_loop.AgentLoop`
...
from pathmind.agents._shared.tool_composition import (
    ToolMountFlags,
...
from pathmind.agents._shared.tool_runtime import (
    bind_workspace_tool_runtime,
    drop_unconfigured_generation_tools,
)
from pathmind.agents.loop.agent_loop import AgentLoop
from pathmind.agents.loop.context_budget import LLMRequestSnapshot, build_context_budget
from pathmind.agents.loop.prompt_blocks import LoopPromptAssembler
from pathmind.capabilities import (
    LoopExtension,
    PromptBlock,
    active_loop_capabilities,
    any_exclusive_capability_active,
)
from pathmind.capabilities.protocol import END_LOOP
from pathmind.core.context import UnifiedContext
from pathmind.core.tool_protocol import ToolLookup
from pathmind.core.trace import (
    build_trace_metadata,
    derive_trace_metadata,
    merge_trace_metadata,
    new_call_id,
)
from pathmind.knowledge.manifest import KbManifest, render_manifest_note
from pathmind.runtime.agentic import (
    DispatchOutcome,
    LLMClientConfig,
    UsageTracker,
...
from pathmind.runtime.agentic.tool_dispatch import (
    MAX_PARALLEL_TOOL_CALLS,
    tool_error_message_factory,
)
from pathmind.runtime.providers import ToolScope
from pathmind.runtime.providers.view import ProviderToolView, build_tool_view
from pathmind.runtime.registry.deferred_tools import DeferredToolLoader
from pathmind.runtime.registry.tool_registry import get_tool_registry
from pathmind.runtime.stream_bus import StreamBus
from pathmind.services.config import get_chat_params
from pathmind.services.llm import (
    get_llm_config,
    get_token_limit_kwargs,  # noqa: F401  (re-exported for tests)
    prepare_multimodal_messages,
    supports_tools,  # noqa: F401  (re-exported for tests)
)
from pathmind.services.llm.context_window import resolve_effective_context_window
from pathmind.services.prompt import get_prompt_manager
from pathmind.services.prompt.lookup import prompt_text as _prompt_text
from pathmind.tools.builtin import PARTNER_BUILTIN_TOOL_NAMES
...
class AgenticLoopPipeline:
    ...
    async def _pageindex_sdk_tool_bundles(self, context: UnifiedContext):
        from pathmind.multi_user.knowledge_access import resolve_kb
        from pathmind.services.rag.factory import PAGEINDEX_OSS_PROVIDER, PAGEINDEX_PROVIDER
        from pathmind.services.rag.pipelines.pageindex.tools import build_sdk_tool_bundle
        from pathmind.services.rag.provider_binding import resolve_bound_provider
    ...
    async def _exec_allowed(self, context: UnifiedContext) -> bool:
        try:
            from pathmind.services.sandbox import IsolationLevel, get_sandbox_service
            ...
            if level is IsolationLevel.SYSTEM:
                ...
                from pathmind.multi_user.tool_access import exec_override
    ...
    def _notebook_choices_full() -> list[dict[str, Any]]:
        try:
            from pathmind.services.notebook import get_notebook_manager
    ...
    async def _execute_tool_call(
        ...
        *,
        ...
        from pathmind.runtime.agentic import execute_tool_call
    ...
    def _estimate_messages_tokens(messages: list[dict[str, Any]]) -> int:
        from pathmind.services.session.context_builder import count_tokens
    ...
    def _current_owner_id() -> str:
        ...
        """
        try:
            from pathmind.multi_user.paths import current_owner_id
    ...
    def _rag_kbs(self, context: UnifiedContext) -> list[str]:
        ...
        from pathmind.services.rag.pipelines.pageindex import is_pageindex_kb
    ...
    def _collect_kb_manifests(kbs: list[str]) -> list[KbManifest]:
        from pathmind.multi_user.knowledge_access import resolve_kb_manifest
    ...
    def _workspace_system_note(self, context: UnifiedContext) -> str:
        ...
        if workspace is None:
            ...
            return ""
        from pathmind.agents._shared.workspace_prompt import workspace_system_note
