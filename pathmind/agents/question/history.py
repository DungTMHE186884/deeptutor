...
* :func:`load_session_quiz_history` — async, takes ``session_id`` and an
  ...
  :class:`~pathmind.agents.question.pipeline.QuizHistoryEntry`.
...
from pathmind.agents.question.pipeline import QuizHistoryEntry
...
async def load_session_quiz_history(
    ...
    *,
    ...
    if not session_id or max_entries <= 0:
        return []
    try:
        from pathmind.services.session.sqlite_store import get_sqlite_session_store
