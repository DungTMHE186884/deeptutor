...
from pathmind.runtime.request_contracts import get_capability_request_schema
...
class DeepQuestionCapability(TurnCapability):
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        ...
        if isinstance(followup_question_context, dict) and followup_question_context.get(
            ...
            from pathmind.agents.question.agents.followup_agent import FollowupAgent
        ...
        if mode != "mimic":
            ...
            from pathmind.agents.question.history import load_session_quiz_history
            from pathmind.agents.question.pipeline import QuestionPipeline
            from pathmind.agents.question.request_config import (
                build_question_runtime_config,
            )
            from pathmind.services.config import load_config_with_main
    ...
    async def _run_mimic_mode(
        ...
        * ``[Attached Documents]`` in  → no paper available; fall back to
        ...
        """
        from pathmind.agents.question.history import load_session_quiz_history
        from pathmind.agents.question.mimic_source import (
        ...
        from pathmind.services.config import load_config_with_main
        from pathmind.services.parsing.engines.mineru.config import MinerUError
