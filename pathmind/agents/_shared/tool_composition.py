...
from pathmind.tools.builtin import (
    BUILTIN_TOOL_NAMES,
...
def default_optional_tools(excluded: Iterable[str] = ()) -> list[str]:
    ...
    Sourced from :mod:`pathmind.tools.builtin` so the /settings/tools UI
...
def compose_enabled_tools(
    ...
    *adds* its owned tools — it never curates or suppresses the reused
    ...
    :class:`~pathmind.capabilities.protocol.KnowledgeCapability`): the turn
...
def user_has_mastery_topics() -> bool:
    ...
    """
    try:
        from pathmind.learning.navigation import learner_has_topics
