"""
Shared prompt loader for non-BaseAgent call sites in ``pathmind/book``.

All LLM-facing prompts inside this module live as YAML under
``pathmind/book/prompts/{en,zh}/<name>.yaml`` and are loaded through the
unified :class:`~pathmind.services.prompt.PromptManager`. This helper is the
...
def load_book_prompts(name: str, language: str) -> dict[str, Any]:
    ...
    Args:
        name: File stem under ``pathmind/book/prompts/{lang}/``
    ...
    if not prompts:
        raise RuntimeError(
            ...
            f"Expected pathmind/book/prompts/{{en,zh}}/{name}.yaml."
        )
    return prompts
