...
def prepare_workspace_execution_env(
    ...
    *,
    ...
    root = turn_root / ".pathmind" / "execution"
    cursor = turn_root
    for part in (".pathmind", "execution"):
        cursor /= part
    ...
    env = {
        ...
        # when a source install is running PathMind from a writable venv.
    ...
    if workspace_root:
        ...
        env["PATHMIND_WORKSPACE_ROOT"] = str(Path(workspace_root).expanduser().resolve())
    return env
