...
def _feature_path(paths, feature: str) -> Path:
    if feature == "attachments":
        ...
        from pathmind.services.storage.attachment_store import _attachment_root
...
def _sessions(paths) -> list[dict]:
    if _backend() == "pocketbase":
        from pathmind.services.session.pocketbase_store import (
            _current_user_id,
            _in_workspace,
            _pb,
            _workspace_filter,
...
def _shared_source_blockers(source_id: str, selected: set[str]) -> list[str]:
    ...
    if source_id or not AUTH_ENABLED or not get_current_user().is_admin:
        return []
    from pathmind.multi_user.book_permission import normalize_book_permission
    from pathmind.multi_user.grants import load_grant
...
def preview(
    ...
    with workspace_context(source_id):
        ...
        from pathmind.services.workspace.dependencies import dependency_closure
        ...
        if "knowledge_bases" in selected:
            from pathmind.services.workspace.resources import knowledge_migration_blockers
        ...
        if _backend() == "pocketbase" and session_ids:
            from pathmind.services.session.pocketbase_store import _pb
...
def migrate_data(
    ...
    from pathmind.services.workspace.session_transfer import transfer_sessions
...
def _pocketbase_rebind(snapshot: Path, target_id: str | None) -> None:
    from pathmind.services.session.pocketbase_store import _json_loads, _pb
    from pathmind.services.workspace.references import rebind_local_urls
...
def _clear_store_caches() -> None:
    from pathmind.learning import storage as learning
    from pathmind.services.session import sqlite_store
    from pathmind.services.storage import file_library
