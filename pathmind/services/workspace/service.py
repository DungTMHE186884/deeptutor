...
from pathmind.core.context import WorkspaceRuntimeContext
from pathmind.multi_user.context import get_current_user
from pathmind.multi_user.paths import get_account_path_service as get_path_service
from pathmind.services.settings.interface_settings import atomic_update
from pathmind.services.workspace.catalog import WorkspaceCatalogMixin
from pathmind.services.workspace.models import WorkspaceBinding, WorkspaceError, WorkspaceItem
from pathmind.utils.secret_files import ensure_private_directory

_SETTINGS_VERSION = 1
_SETTINGS_NAME = "content_workspace"
_DEPLOYMENT_ROOT_ENV = "PATHMIND_WORKSPACE_ROOT"
_ALLOWED_ROOTS_ENV = "PATHMIND_WORKSPACE_ALLOWED_ROOTS"
_INTERNAL_DIR = ".pathmind"
...
def _workspace_id(user_id: str, root: Path) -> str:
    value = f"pathmind-workspace:{user_id}:{root}"
...
def _workspace_item_id(workspace_id: str, relative_path: str, sha256: str) -> str:
    ...
    """
    value = f"pathmind-workspace-item:{workspace_id}:{relative_path}:{sha256}"
...
class ContentWorkspaceService(WorkspaceCatalogMixin):
    ...
    def security_level() -> str:
        from pathmind.services.sandbox.config import SandboxSettings, build_backend
        from pathmind.services.sandbox.spec import IsolationLevel
    ...
    def _ensure_ready(self, binding: WorkspaceBinding) -> None:
        ...
        try:
            ...
            probe = output / f".pathmind-write-probe-{uuid.uuid4().hex}"
    ...
    def _presentation_root(binding: WorkspaceBinding, *, create: bool = False) -> Path:
        ...
        from pathmind.services.path_service import get_path_service as get_scoped_paths
        from pathmind.services.workspace.context import current_workspace_id
    ...
    def list_entries_page(
        ...
        *,
        ...
        if (
            ...
            raise WorkspaceError("PathMind's internal workspace files are not listable.")
    ...
    def read_text(
        ...
        *,
        ...
        if _INTERNAL_DIR in PurePosixPath(_normalise_relative(path)).parts:
            raise WorkspaceError("PathMind's internal workspace files are not readable.")
    ...
    def search_page(
        ...
        *,
        ...
        if (
            ...
            raise WorkspaceError("PathMind's internal workspace files are not searchable.")
        base = binding.root if path in {"", "."} else self.resolve(binding, path)
        if not base.is_dir():
            raise WorkspaceError("The requested workspace path is not a directory.")
        rows: list[dict[str, Any]] = []
    ...
    def publish(
        ...
        from pathmind.services.workspace.context import current_workspace_id, workspace_url
        ...
        for raw in items:
            ...
            if _INTERNAL_DIR in PurePosixPath(relative).parts:
                raise WorkspaceError("PathMind's internal workspace files cannot be presented.")
    ...
    def validate_export(
        ...
        *,
        ...
        if _INTERNAL_DIR in PurePosixPath(destination_relative).parts:
            raise WorkspaceError("PathMind's internal workspace files cannot be overwritten.")
    ...
    def export_once(
        ...
        *,
        ...
        destination = self.resolve(binding, request["destination_path"])
        fd, temp_name = tempfile.mkstemp(prefix=".pathmind-export-", dir=destination.parent)
        os.close(fd)
    ...
    def resolve_published_item(
        ...
        from pathmind.services.workspace.context import current_workspace_id, get_workspace_scope
