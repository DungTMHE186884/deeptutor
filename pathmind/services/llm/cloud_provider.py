...
Chat traffic goes through :mod:`pathmind.services.llm.factory` and the
...
def _warn_deprecated(name: str) -> None:
    warnings.warn(
        f"pathmind.services.llm.cloud_provider.{name} is deprecated; "
        "use pathmind.services.llm.complete / stream",
        DeprecationWarning,
        stacklevel=3,
    )
...
async def complete(prompt: str, **kwargs: Any) -> str:
    """Deprecated: forwards to :func:`pathmind.services.llm.factory.complete`."""
...
async def stream(prompt: str, **kwargs: Any) -> AsyncGenerator[str, None]:
    """Deprecated: forwards to :func:`pathmind.services.llm.factory.stream`."""
