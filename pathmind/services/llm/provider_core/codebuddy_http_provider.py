...
from pathmind.services.codebuddy_credentials import (
...
from pathmind.services.llm.provider_core.base import LLMResponse
...
class CodeBuddyHTTPProvider(OpenAICompatProvider):
    ...
    async def _chat_impl(
        ...
        **kwargs: Any,
        ...
        kwargs.pop("pathmind_session_id", None)
...
def build_codebuddy_provider(
    ...
    *,
    ...
    ``PATHMIND_CODEBUDDY_BACKEND`` (``http`` / ``sdk``) forces one transport;
    the SDK is still needed for CLI-driven agent features such as sandboxed
    shell tools.
    """
    preference = (os.environ.get("PATHMIND_CODEBUDDY_BACKEND") or "").strip().lower()
    ...
    from pathmind.services.llm.provider_core.codebuddy_provider import CodeBuddyProvider
