...
def _charge_current_user(record: dict[str, Any]) -> None:
    """Charge this call's credits to the signed-in user (subscription quota).

    Runs for every measured call — chat turns, partners, book generation, … —
    so no LLM path escapes billing. Calls outside a user context (CLI,
    background jobs) are not charged. Never raises into the LLM path.
    """
    try:
        ...
        from pathmind.services.quota_service import record_llm_call

        with get_db_session() as db:
            record_llm_call(user.id, record, db)
    except Exception:
        logging.getLogger(__name__).warning("Unable to charge LLM usage to user", exc_info=True)
