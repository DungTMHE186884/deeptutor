...
from pathmind.agents._shared.tool_composition import default_optional_tools
...
BUILTIN_PARTNER_COMMANDS: tuple[PartnerCommandSpec, ...] = (
    ...
    PartnerCommandSpec(
        "/link",
        "Connect this chat account to your PathMind account.",
...
class PartnerCommandHandler:
    ...
    def _link(self, msg: InboundMessage, args: list[str]) -> PartnerCommandResult:
        ...
        from pathmind.services.partners.links import redeem_link_code
        ...
        if not args:
            return PartnerCommandResult(
                "Usage: /link <code>. Open this partner in PathMind and choose "
        ...
        if not user_id:
            return PartnerCommandResult(
                ...
                "Generate a fresh one in PathMind and try again."
            )
