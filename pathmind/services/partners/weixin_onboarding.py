...
This drives the same exchange (:mod:`pathmind.partners.channels.weixin_qr`) on
...
from pathmind.partners.channels.weixin_qr import (
    QrOutcome,
    fetch_qr_code,
...
def _client_version() -> int:
    from pathmind.partners.channels.weixin import ILINK_APP_CLIENT_VERSION

    return ILINK_APP_CLIENT_VERSION


def _channel_config(partner_id: str) -> dict[str, Any]:
    from pathmind.services.partners.manager import get_partner_manager
...
async def _apply_outcome(attempt: _Attempt, outcome: QrOutcome) -> None:
    ...
    if outcome.status == "confirmed":
        try:
            ...
            attempt.status = "error"
            attempt.error = (
                "WeChat confirmed the login, but PathMind could not save or start "
