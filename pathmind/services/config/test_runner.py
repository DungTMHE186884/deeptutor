...
class ConfigTestRunner:
    ...
    async def _test_llm(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from pathmind.services.llm.factory import complete_with_config
    ...
    async def _test_embedding(
        ...
        probe_texts = [
            "PathMind embedding smoke test",
            "PathMind retrieval batch probe",
        ]
    ...
    def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from pathmind.services.settings.provider_probe import test_search_access
    ...
    async def _test_tts(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from pathmind.services.config.provider_runtime import resolve_tts_runtime_config
        from pathmind.services.voice import synthesize_speech
        ...
        sample = "PathMind voice check. 这是一段语音合成测试。"
    ...
    async def _test_stt(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from pathmind.services.config.provider_runtime import resolve_stt_runtime_config
        from pathmind.services.voice import transcribe_audio
    ...
    async def _test_imagegen(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from pathmind.services.config.provider_runtime import resolve_imagegen_runtime_config
        from pathmind.services.imagegen import generate_image
    ...
    async def _test_videogen(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from pathmind.services.config.provider_runtime import resolve_videogen_runtime_config
        from pathmind.services.videogen import probe_video
