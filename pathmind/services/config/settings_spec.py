"""Machine-readable description of what a PathMind install can be told to be.
...
(:mod:`pathmind.capabilities.setup.tools`) completely generic: they walk this
...
``scope``
    ...
    row is one that :class:`~pathmind.services.path_service.PathService`
    already resolves per user, a global row is one read through
    :func:`~pathmind.services.config.loader.get_runtime_settings_dir`.
...
``probe``
    ...
    :func:`~pathmind.capabilities.setup.apply.apply_setting` refuses to commit
...
Imports of the rest of PathMind are deferred into the row callables on
purpose: this module is imported from ``pathmind.capabilities``, and a
...
def _read_ui(key: str, default: str) -> Callable[[], str]:
    def _read() -> str:
        from pathmind.services.settings.interface_settings import get_ui_settings
...
def _write_ui(key: str) -> Callable[[str], None]:
    def _write(value: str) -> None:
        from pathmind.services.settings.interface_settings import set_ui_setting
...
def _interface_specs() -> list[SettingSpec]:
    ...
    return [
        SettingSpec(
            ...
            summary="Language of the PathMind UI (menus, buttons, settings).",
            read=language_read,
...
def _read_parsing_engine() -> str:
    from pathmind.services.config.runtime_settings import get_runtime_settings_service
...
def _parsing_engine_choices() -> tuple[SettingChoice, ...]:
    ...
    """
    from pathmind.services.parsing.engines._install import installable_engines
    from pathmind.services.parsing.engines.factory import list_engines
...
def _validate_parsing_engine(value: str) -> str | None:
    from pathmind.services.parsing.engines.factory import is_engine_available
...
def _catalog_service():
    from pathmind.services.config.model_catalog import get_model_catalog_service
...
def _catalog_profile_choices(service_name: str) -> Callable[[], tuple[SettingChoice, ...]]:
    def _choices() -> tuple[SettingChoice, ...]:
        from pathmind.services.config.provider_runtime import SEARCH_PROVIDERS
...
def _clear_runtime_caches() -> None:
    ...
    """
    try:
        from pathmind.services.llm import clear_llm_config_cache
...
async def _probe_llm(value: str) -> ProbeResult:
    ...
    from pathmind.services.config.provider_runtime import resolve_llm_runtime_config
    from pathmind.services.llm import complete as llm_complete
    from pathmind.services.llm import get_token_limit_kwargs
...
async def _probe_embedding(value: str) -> ProbeResult:
    ...
    from pathmind.services.config.provider_runtime import resolve_embedding_runtime_config
    from pathmind.services.embedding.client import EmbeddingClient
    from pathmind.services.embedding.config import EmbeddingConfig
...
def setting_specs() -> dict[str, SettingSpec]:
    ...
    ``pathmind.capabilities`` without closing an import cycle.
    """
