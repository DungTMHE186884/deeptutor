...
Readiness is not the same thing as correctness.  Most of what PathMind can be
...
from pathmind.services.config.model_catalog import SERVICE_NAMES
...
async def _selected_remote_parser_reachable(
    ...
    try:
        if engine_id == "tika":
            from pathmind.services.parsing.engines.tika.remote import (
        ...
        if engine_id == "docling" and bool(getattr(config, "is_remote", False)):
            from pathmind.services.parsing.engines.docling.remote import (
...
async def _redis_reachable(redis_url: str) -> bool:
    ...
    coordinator = None
    try:
        from pathmind.runtime.coordination.redis import RedisCoordinator
...
async def build_settings_readiness() -> dict[str, Any]:
    ...
    from pathmind.multi_user.knowledge_access import current_kb_manager
    from pathmind.services.config import get_model_catalog_service, get_runtime_settings_service
    from pathmind.services.parsing.engines.factory import get_parser, list_engines
    from pathmind.services.rag.preflight import engine_preflight
    from pathmind.services.settings.interface_settings import get_enabled_optional_tools
    from pathmind.video_learning import load_video_learning_settings
    from pathmind.visualizers.registry import get_visualizer_registry
