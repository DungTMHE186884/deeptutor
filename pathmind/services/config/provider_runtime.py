"""Nanobot-style normalized runtime configuration for PathMind."""
...
from pathmind.services.imagegen.config import ImagegenConfig
from pathmind.services.keypool import primary_api_key
from pathmind.services.model_selection import LLMSelection, apply_llm_selection_to_catalog
...
from pathmind.services.videogen.config import VideogenConfig
from pathmind.services.voice.config import (
    AUTH_API_KEY_HEADER,
    AUTH_BEARER,
    STT_BASE64_JSON,
    STT_MULTIPART,
...
class SearchProviderSpec:
    ...
    its own answer or needs :mod:`pathmind.services.search.consolidation` to
    ...
    The provider registry (``pathmind/services/search/providers``), the
    Settings dropdown (``pathmind/api/routers/settings.py``) and the web
...
def _with_personal_llm_profiles(catalog: dict[str, Any]) -> dict[str, Any]:
    ...
    the shared one (see :mod:`pathmind.multi_user.personal_models`), so
...
def _register_catalog_capabilities(catalog: dict[str, Any]) -> None:
    ...
    without a restart. Imported lazily: ``pathmind.services.llm`` imports this
    module for its config, so a module-level import would close a cycle.
    """
    from pathmind.services.llm.capabilities import set_catalog_capability_overrides
...
def resolve_llm_runtime_config(
    ...
    *,
    ...
    is the same shape and stands in for it on the calls PathMind makes on its
...
def resolve_tts_runtime_config(
    ...
    *,
    ...
    if not api_key and spec.is_local:
        api_key = "sk-no-key-required"
    from pathmind.services.voice.options import voice_model_options
