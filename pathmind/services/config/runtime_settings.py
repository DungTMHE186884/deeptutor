...
from pathmind.services.file_io import atomic_write_json as _atomic_write_json
...
DEFAULT_SYSTEM_SETTINGS: dict[str, Any] = {
    ...
    # audited deployments; PATHMIND_VERSION_CHECK_ENABLED is the deployment
    # override for read-only settings volumes. PathMind is a fork: the release
    # feed still points at upstream DeepTutor, so the check is off by default.
...
# Document parsing settings. The parse layer (pathmind/services/parsing)
...
_DEFAULT_MINERU_ENGINE: dict[str, Any] = {
    ...
    # separate conda) so its heavy deps never conflict with PathMind's.
    "local_cli_path": "",
...
# PathMind's VLM to describe images.
...
# PathMind's legacy text extractors for PDF / Office / text-like files.
...
# ``max_concurrent_files`` sizes the native parser worker pool after PathMind
...
IGNORE_PROCESS_OVERRIDES_ENV = "PATHMIND_IGNORE_PROCESS_ENV_OVERRIDES"
...
class RuntimeSettingsService:
    ...
    def render_environment(self) -> dict[str, str]:
        ...
        return {
            "PATHMIND_VERSION_CHECK_ENABLED": _bool_env(system["version_check_enabled"]),
            ...
            "PATHMIND_SANDBOX_ALLOW_SUBPROCESS": _bool_env(system["sandbox_allow_subprocess"]),
            ...
            # rewrites /api/* and /ws/* to PATHMIND_API_BASE_URL and uses
            # PATHMIND_AUTH_ENABLED to gate the login redirect. The launcher and
            # the Docker entrypoint both export these through render_environment,
            # so the two deployment paths stay in sync. PATHMIND_API_BASE_URL is
    ...
    def _apply_system_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if value := self._process_env_value("PATHMIND_VERSION_CHECK_ENABLED"):
        ...
        if value := (
            self._process_env_value("PATHMIND_BACKEND_WORKERS")
            or self._process_env_value("BACKEND_WORKERS")
        ):
            payload["backend_workers"] = value
        if value := self._process_env_value("PATHMIND_SANDBOX_ALLOW_SUBPROCESS"):
            payload["sandbox_allow_subprocess"] = value
        if value := self._process_env_value("PATHMIND_CAPABILITY_ROUTING_ENABLED"):
    ...
    def _apply_integrations_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if value := self._process_env_value("PATHMIND_TURN_COORDINATION_BACKEND"):
            coordination["backend"] = value
        if value := self._process_env_value("PATHMIND_REDIS_URL"):
            coordination["redis_url"] = value
        if value := self._process_env_value("PATHMIND_REDIS_KEY_PREFIX"):
    ...
    def _apply_llamaindex_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        # (PATHMIND_RAG_RETRIEVAL_PROFILE / RAG_RETRIEVAL_PROFILE); preserve it.
        payload = dict(settings)
        if value := (
            self._process_env_value("PATHMIND_RAG_RETRIEVAL_PROFILE")
    ...
    def _normalize_system(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "version": 1,
            "version_check_enabled": _coerce_bool(settings.get("version_check_enabled"), False),
    ...
    def _normalize_integrations(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if backend not in {"memory", "redis"}:
            backend = "memory"
        key_prefix = _string(coordination.get("key_prefix")).strip(":") or "pathmind"
        return {
            "version": 2,
...
def get_chat_attachment_limits() -> ChatAttachmentLimits:
    ...
    max_file = int(system["chat_attachment_max_file_mb"]) * 1024 * 1024
    max_total = int(system["chat_attachment_max_total_mb"]) * 1024 * 1024
    plan_cap = _plan_upload_cap_bytes()
    if plan_cap is not None and plan_cap > 0:
        # The subscription plan narrows the system ceiling for this user.
        max_file = min(max_file, plan_cap)
        max_total = max(max_file, min(max_total, plan_cap * 5))
...
def _plan_upload_cap_bytes() -> int | None:
    """Per-file upload cap of the current user's plan (None = no limit/unknown).

    Admins and requests without a signed-in user keep the system ceiling.
    Never raises: attachment handling must not break on a billing DB issue.
    """
    try:
        from pathmind.multi_user.context import get_current_user_or_none

        user = get_current_user_or_none()
        if user is None or getattr(user, "role", "user") == "admin":
            return None
        from pathmind.database.connection import get_db_session
        from pathmind.services.quota_service import get_user_plan

        with get_db_session() as db:
            plan = get_user_plan(user.id, db)
            return int(plan.max_upload_file_size_bytes or 0) or None
    except Exception:
        return None
