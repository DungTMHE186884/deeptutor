...
from .model_catalog import (
    decrypt_catalog_secrets,
    encrypt_catalog_secrets,
...
class SettingsDraftService:
    ...
    def load(self) -> dict[str, Any]:
        ...
        if not isinstance(draft.get("extensions"), dict):
            draft["extensions"] = {}
        if isinstance(draft.get("catalog"), dict):
            draft["catalog"], _ = decrypt_catalog_secrets(draft["catalog"])
        return draft
    ...
    def save(self, draft: dict[str, Any]) -> dict[str, Any]:
        with self._lock:
            ...
            try:
                on_disk = dict(normalized)
                if isinstance(on_disk.get("catalog"), dict):
                    # Keys typed into an unapplied draft are encrypted too.
                    on_disk["catalog"] = encrypt_catalog_secrets(on_disk["catalog"])
