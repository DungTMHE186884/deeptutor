...
from pathmind.services.provider_registry import (
...
def _destination(profile: dict[str, Any]) -> tuple[str, str]:
    """Where a profile's credentials are sent: API host and proxy."""
    from urllib.parse import urlsplit

    raw = str(profile.get("base_url") or "").strip()
    try:
        parts = urlsplit(raw if "://" in raw else f"https://{raw}") if raw else None
        host = f"{parts.scheme.lower()}://{(parts.netloc or '').lower()}" if parts else ""
    except ValueError:
        host = raw.lower()
    return host, str(profile.get("proxy") or "").strip().lower()


def _restore_profile_secrets(proposed: dict[str, Any], current: dict[str, Any]) -> None:
    # A masked secret only means "keep the stored key" while the key keeps
    # going to the same place. If the API host or proxy changed, the stored
    # key is NOT carried over — otherwise anyone with settings access could
    # point a connection at their own server and receive the real key.
    moved = _destination(proposed) != _destination(current)
    for key, value in list(proposed.items()):
        if key == "extra_headers" or _is_secret_field(key):
            if moved and value == CATALOG_SECRET_MASK and current.get(key):
                logger.warning(
                    "Endpoint of %r changed: its stored %s was cleared and must be re-entered.",
                    proposed.get("name") or proposed.get("id"),
                    key,
                )
                proposed[key] = ""
                continue
            if moved and key == "extra_headers" and isinstance(value, dict):
                value = {k: ("" if v == CATALOG_SECRET_MASK else v) for k, v in value.items()}
...
def _iter_secret_holders(catalog: dict[str, Any]):
...
def _secret_keys(holder: dict[str, Any]):
    return [key for key in holder if key == "extra_headers" or _is_secret_field(key)]


def encrypt_catalog_secrets(catalog: dict[str, Any]) -> dict[str, Any]:
    """Copy of *catalog* with every credential encrypted for storage."""
    from pathmind.services import secret_box

    out = deepcopy(catalog)
    for holder in _iter_secret_holders(out):
        for key in _secret_keys(holder):
            holder[key] = secret_box.transform(holder[key], secret_box.encrypt_value)
    return out


def decrypt_catalog_secrets(catalog: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    """Decrypt stored credentials in place; also report leftover plain text."""
    from pathmind.services import secret_box

    had_plaintext = False
    for holder in _iter_secret_holders(catalog):
        for key in _secret_keys(holder):
            value = holder[key]
            if secret_box.contains_plaintext(value) and value != CATALOG_SECRET_MASK:
                had_plaintext = True
            holder[key] = secret_box.transform(value, secret_box.decrypt_value)
    return catalog, had_plaintext
...
def _normalize_profile_api_format(profile: dict[str, Any]) -> bool:
    ...
    still written because a downgraded PathMind reads only that field, and
...
class ModelCatalogService:
    ...
    def load(self) -> dict[str, Any]:
        ...
        if loaded:
            ...
            if merged_defaults or catalog != before or self._needs_encryption:
                # Also migrates keys still stored in plain text.
                self.save(catalog)
            return catalog
    ...
    def _read_existing_catalog(self) -> dict[str, Any]:
        ...
        if not isinstance(loaded, dict):
            return {}
        loaded, self._needs_encryption = decrypt_catalog_secrets(loaded)
        return loaded
        ...
        with self._lock:
            ...
            try:
                with os.fdopen(fd, "w", encoding="utf-8", newline="\n") as handle:
                    # Credentials are encrypted on disk; callers keep plain values.
                    json.dump(encrypt_catalog_secrets(normalized), handle, indent=2, ensure_ascii=False)
                    handle.write("\n")
                    handle.flush()
