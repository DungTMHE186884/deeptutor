...
from pathmind.services.cron.service import CronJob
...
async def _maybe_send_desktop_notification(job: CronJob, text: str) -> None:
    ...
    PathMind still persists/delivers through its normal chat or partner
    ...
    title = f"PathMind: {job.name or 'Reminder'}"
...
async def _execute_partner_job(job: CronJob) -> tuple[str, str | None]:
    ...
    if not delivery_meta.get("_streamed"):
        from pathmind.partners.bus.events import OutboundMessage
