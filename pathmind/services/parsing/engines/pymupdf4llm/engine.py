...
class PyMuPDF4LLMParser:
    ...
    def is_ready(self, config: PyMuPDF4LLMConfig) -> ReadinessReport:
        if not self.is_available():
            return ReadinessReport(
                ...
                message="pymupdf4llm isn't installed (pip install pathmind[parse-pymupdf4llm]).",
            )
        ...
        if not pymupdf4llm_version_is_current(version):
            return ReadinessReport(
                ...
                message=(
                    f"Installed PyMuPDF4LLM {version or 'unknown'} is too old. PathMind needs "
    ...
    def _resolve_to_markdown() -> Callable[..., object]:
        ...
        so PathMind should use it instead of pinning the legacy helper and
