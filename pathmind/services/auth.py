"""
Authentication service for PathMind.
...
Quick setup (single user via data/user/settings/auth.json):
    ...
    3. Generate a password hash:
           python -c "from pathmind.services.auth import hash_password; print(hash_password('yourpassword'))"
...
from pathmind.multi_user.models import AccountPreset, Role
from pathmind.services.config import load_auth_settings, load_integrations_settings
...
def _make_user_record(
    ...
    from pathmind.multi_user.identity import new_user_id
...
def _load_users() -> dict[str, dict]:
    ...
    """
    from pathmind.multi_user.identity import load_users
...
def add_user(
    ...
    """
    from pathmind.multi_user.identity import save_user
...
def list_users() -> list[dict]:
    ...
    from pathmind.multi_user.identity import list_user_info

    return list_user_info(AUTH_USERNAME, AUTH_PASSWORD_HASH)


def is_locked_account(username: str, password: str) -> bool:
    """True when the credentials are right but the account is locked.

    Only answers after a correct password, so it never reveals which
    usernames exist or are locked.
    """
    users = _load_users()
    record = users.get(username)
    if not record and "@" in (username or ""):
    ...
    if not isinstance(record, dict) or not record.get("disabled"):
        return False
    return verify_password(password, str(record.get("hash") or ""))
...
def delete_user(username: str) -> bool:
    ...
    """
    from pathmind.multi_user.identity import delete_user as _delete_user
...
def set_role(username: str, role: str) -> bool:
    ...
    from pathmind.multi_user.identity import set_role as _set_role
...
def set_avatar(username: str, avatar: str) -> bool:
    ...
    """
    from pathmind.multi_user.identity import set_avatar as _set_avatar
...
def get_learner_profile(username: str) -> dict[str, Any] | None:
    ...
    from pathmind.multi_user.identity import get_learner_profile as _get_profile
...
def set_learner_profile(username: str, profile: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    from pathmind.multi_user.identity import set_learner_profile as _set_profile
...
_ACCOUNT_INDEX: dict = {"key": None, "loaded_at": 0.0, "accounts": {}}
_ACCOUNT_INDEX_MAX_AGE_S = 2.0


def _account_is_active(username: str, user_id: str) -> bool:
    """A token is only honoured while its account still exists and is enabled.

    Deleting (or locking) an account must end its sessions immediately
    instead of when the JWT expires. The user store is re-read when the file
    (path, size or mtime) changes, and at least every couple of seconds —
    file timestamps are too coarse on some filesystems to catch two quick
    writes on their own.
    """
    import time

    from pathmind.multi_user import identity

    path = identity.USERS_FILE
    try:
        ...
        key = (str(path), 0, 0)
    now = time.monotonic()
    if _ACCOUNT_INDEX["key"] != key or now - _ACCOUNT_INDEX["loaded_at"] > _ACCOUNT_INDEX_MAX_AGE_S:
        _ACCOUNT_INDEX["accounts"] = {
            name: (str(rec.get("id") or ""), bool(rec.get("disabled", False)))
            for name, rec in _load_users().items()
            if isinstance(rec, dict)
        }
        _ACCOUNT_INDEX["key"] = key
        _ACCOUNT_INDEX["loaded_at"] = now
    entry = _ACCOUNT_INDEX["accounts"].get(username)
    if entry is None:
        return False
    stored_id, disabled = entry
    return not disabled and (not user_id or not stored_id or stored_id == user_id)
...
def decode_token(token: str) -> TokenPayload | None:
    ...
    if POCKETBASE_ENABLED:
        from pathmind.services.pocketbase_client import validate_pb_token
    ...
    try:
        ...
        if not user_id:
            ...
            user_id = str(record.get("id") or "")
        if not _account_is_active(str(username), user_id):
            return None
        device_credential_id = str(payload.get("dcid") or "")
        device_session_nonce = str(payload.get("dcs") or "")
        if device_credential_id:
            from pathmind.multi_user.device_credentials import validate_device_token
...
def authenticate_pb(username: str, password: str) -> tuple[TokenPayload, str] | None:
    ...
    <username>@pathmind.local to match the email used at registration.
    """
    try:
...
def authenticate(username: str, password: str) -> TokenPayload | None:
    ...
    record = users.get(username)
    if not record and "@" in (username or ""):
        # Accounts can also sign in with the email they registered.
        from pathmind.multi_user.identity import find_user_by_email
...
def authenticate_device(pairing_code: str, pin: str) -> TokenPayload | None:
    ...
    if not AUTH_ENABLED or POCKETBASE_ENABLED:
        return None
    from pathmind.multi_user.device_credentials import begin_device_session
