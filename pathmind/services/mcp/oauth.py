...
sibling module :mod:`pathmind.services.mcp.secrets` holds static credentials in
...
#: How PathMind introduces itself on a consent screen.
CLIENT_NAME = "PathMind"
...
def _store_path(owner_id: str, server: str) -> Path:
    from pathmind.multi_user.paths import owner_secrets_dir
...
_PUBLIC_URL_ENV = "PATHMIND_PUBLIC_URL"
...
def oauth_redirect_uri(origin: str = "") -> str:
    ...
    *origin* is the origin the person is actually browsing, which the route that
    ...
    deployment work with no configuration. ``PATHMIND_PUBLIC_URL`` overrides it
...
def client_metadata(redirect_uri: str) -> "Any":
    """How PathMind registers itself with an authorization server."""
