...
A hub skill is the same artefact PathMind already speaks natively — a
...
# PathMind ships pointing at EduHub, its native open skill registry: a bare
...
class ClawHubProvider:
    ...
    def fetch(self, slug: str, *, version: str | None = None) -> FetchedSkill:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="pathmind-skill-"))
        try:
    ...
    def catalog(
        ...
        PathMind-native browser renders without a second round-trip.
        """
        if query.strip():
