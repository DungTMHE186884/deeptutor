"""Local and opt-in online diagnostics for the PathMind CLI."""
...
from pathmind.services.llm.utils import is_local_llm_server, sanitize_url
from pathmind.services.provider_registry import find_by_name
...
class DoctorCheck:
    """One diagnostic result shown by ``pathmind doctor``."""
...
def _rag_check(
    ...
    for kb_name, entry in knowledge_bases.items():
        ...
        if provider == WEKNORA_PROVIDER:
            from pathmind.services.rag.pipelines.weknora.config import (
        ...
        from pathmind.services.rag.pipelines.lightrag_server.config import (
