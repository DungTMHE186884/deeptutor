...
from pathmind.services.prompt.language import is_chinese as _is_zh
...
def _render_l3(cap: int) -> str:
    ...
    Reads through :class:`~pathmind.services.memory.store.MemoryStore`'s
...
def _collect_material(trace_count: int) -> _Material:
    ...
    nothing about that.
    """
    from pathmind.services.memory import recall
...
async def _generate(language: str, material: _Material) -> SuggestionSet:
    ...
    try:
        ...
        from pathmind.services.model_selection.tasks import TaskKind, task_llm_scope
