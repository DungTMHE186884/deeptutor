...
entry's provenance (see :class:`~pathmind.services.cli_apps.models.AppTrust`).

What this module does *not* do:

* **Run the registry's install command.** It runs an argv assembled from an
  :class:`~pathmind.services.cli_apps.models.InstallPlan`. See ``models``.
...
from pathmind.services.cli_apps.models import (
    AppRuntime,
    CliAppEntry,
    InstallKind,
)
from pathmind.services.cli_apps.paths import (
    abi_stamp,
    app_dir,
    ensure_root,
    executable_path,
...
INSTALL_TIMEOUT_S = int(os.environ.get("PATHMIND_CLI_APP_INSTALL_TIMEOUT_S", "900"))
...
def _pin_of(entry: CliAppEntry) -> str:
    ...
    if entry.install.kind is InstallKind.PINNED_HARNESS:
        from pathmind.services.cli_apps.catalog import catalog_pin
