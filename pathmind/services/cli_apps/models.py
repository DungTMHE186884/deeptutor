    ...
    if not text:
        return InstallPlan(
            ...
            reason=(
                ...
                "there is nothing for PathMind to install."
            ),
        )
    ...
    if any(token in text for token in ("&&", "||", "|", ">", ";", "$(", "`")):
        return InstallPlan(
            ...
            reason=(
                ...
                "single package install. PathMind builds every install command "
    ...
    if len(requirements) != 1:
        return InstallPlan(
            ...
            reason=(
                ...
                "which PathMind does not model as a single app."
            ),
        )
...
def _unsupported_manager(manager: str) -> InstallPlan:
    return InstallPlan(
        ...
        trust=AppTrust.THIRD_PARTY,
        reason=(
            f"Installs with {manager}, which the PathMind runtime image does not "
