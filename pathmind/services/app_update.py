...
from pathmind.runtime.home import get_runtime_home
...
LAUNCHER_PID_ENV = "PATHMIND_LAUNCHER_PID"
...
def _version_tuple(value: str) -> tuple[int, int, int]:
    ...
    if match is None:
        raise ValueError(f"Invalid PathMind version: {value}")
...
def _running_in_container() -> bool:
    configured = os.getenv("PATHMIND_CONTAINER", "").strip().lower()
...
def _distribution_direct_url() -> dict[str, Any] | None:
    try:
        distribution = importlib.metadata.distribution("pathmind")
...
def detect_installation() -> Installation:
    ...
    if direct_url is None:
        return Installation(
            ...
            current_version=__version__,
            automatic_update=False,
            command="pip install -U pathmind",
            reason="The running PathMind distribution could not be identified.",
        )
            ...
            try:
                response = await client.get(
                    ...
                    headers={
                        ...
                        "User-Agent": "PathMind-Version-Check",
                    },
                )
    ...
    async def _fetch_latest_redirect(self, client: httpx.AsyncClient) -> ReleaseInfo:
        ...
        try:
            response = await client.head(
                self._latest_url,
                headers={"User-Agent": "PathMind-Version-Check"},
...
class UpdateJobStore:
    ...
    def create(self, *, current_version: str, target_version: str) -> UpdateJob:
        ...
        if _version_tuple(target) <= _version_tuple(current):
            raise UpdateRequestError("No newer PathMind release is available")
...
def launch_update_worker(store_root: Path, *, parent_pid: int) -> None:
    ...
    command = [
        sys.executable,
        "-m",
        "pathmind.runtime.update_worker",
        "--store-root",
