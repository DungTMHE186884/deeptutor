...
from pathmind.services.llm.cloud_provider import _auth_binding, _get_aiohttp_connector
from pathmind.services.llm.utils import build_auth_headers, collect_model_names
...
def test_search_access(
    ...
    from pathmind.services.search.providers import get_provider
    ...
    response = get_provider(binding, **kwargs).search(
        "PathMind configuration health check", **kwargs
    )
...
async def probe_search_provider(binding: str, base_url: str, api_key: str | None) -> dict:
    from pathmind.services.config.provider_runtime import search_missing_credential
