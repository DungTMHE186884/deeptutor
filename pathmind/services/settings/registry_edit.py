...
from pathmind.services.config.model_catalog import SERVICE_NAMES, restore_catalog_secrets
from pathmind.services.config.provider_links import (
    PROVIDER_FIELDS,
    provider_source,
    referenced_models,
)
...
def _task_choice(result, bucket, choice):
    ...
    if mode == "inherit":
        return {"mode": mode}
    if mode == "reference":
        from pathmind.services.model_selection.llm import apply_llm_selection_to_catalog
...
def merge_registry_edit(current, edit, *, stored_draft=None):
    ...
    if kind == "provider":
        ...
        if task_kind is None:
            ...
            from pathmind.services.model_selection.tasks import TASK_KINDS, TASK_OVERRIDES_KEY
