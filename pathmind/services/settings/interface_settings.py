...
VALID_THEMES = ("snow", "light", "dark", "glass")


def _deployment_default_theme() -> str:
    """Theme for visitors who are not signed in and for brand-new accounts.

    Set ``PATHMIND_DEFAULT_THEME`` (snow | light | dark | glass) to change it
    for a deployment; "snow" is the pure-white theme shown as "Default".
    """
    value = os.environ.get("PATHMIND_DEFAULT_THEME", "").strip().lower()
    return value if value in VALID_THEMES else "snow"


DEFAULT_THEME = _deployment_default_theme()

DEFAULT_UI_SETTINGS: dict[str, Any] = {
    # Accounts that never picked a theme get the deployment default.
    "theme": DEFAULT_THEME,
    "language": "en",
