...
from pathmind.core.stream import StreamEvent, StreamEventType
from pathmind.core.turn_request import TurnRequest
from pathmind.runtime.capability_routing import route_explicit_quiz_request
from pathmind.services.session.workspace_preferences import (
    WORKSPACE_MODE_MASTERY,
    WORKSPACE_MODE_READING,
    WORKSPACE_MODE_WATCHING,
)
from pathmind.services.workspace.activity import workspace_writer
...
if TYPE_CHECKING:
    from pathmind.runtime.coordination import RuntimeCoordinator
    from pathmind.services.session.protocol import SessionStoreProtocol
...
class TurnRequestPreparer:
    ...
    async def start_turn(self, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        from pathmind.services.workspace.context import get_workspace_scope
        ...
        if per_turn_auto_route is None:
            from pathmind.services.config.runtime_settings import load_system_settings
        ...
        if requested_course_id:
            from pathmind.services.courses import (
                CourseNotFoundError,
                get_course_service,
            )
        ...
        try:
            from pathmind.multi_user.learning_access import apply_learning_policy
        ...
        if workspace_mode == WORKSPACE_MODE_WATCHING:
            from pathmind.video_learning import get_timed_media_store
        ...
        try:
            from pathmind.runtime.request_contracts import validate_capability_config
        ...
        if workspace_mode == WORKSPACE_MODE_READING and reading_workspace_id:
            from pathmind.reading import ReadingCatalogStore
        ...
        if workspace_mode == WORKSPACE_MODE_MASTERY:
            from pathmind.learning.identity import resolve_mastery_path_binding
        ...
        from pathmind.capabilities.mastery.mode import enforced_mode
        ...
        if llm_selection:
            ...
            from pathmind.multi_user.context import get_current_user
            from pathmind.multi_user.model_access import (
            ...
            if not current_user.is_admin:
                ...
                from pathmind.multi_user.model_access import plan_default_option

                # Prefer the plan's default model (e.g. a cheap one for Free),
                # otherwise the first granted-and-available model.
                chosen = plan_default_option(current_user.id, assigned_llms) or assigned_llms[0]
                llm_selection = {
                    "profile_id": chosen.get("profile_id"),
                    "model_id": chosen.get("model_id"),
                }
        if llm_selection:
            from pathmind.multi_user.personal_models import merge_personal_llm_profiles
        ...
        # capability's turn.
        from pathmind.multi_user.tool_access import allowed_optional_tools
        ...
        if capability_route is not None and capability_route.auto_routed:
            from pathmind.runtime.registry.capability_registry import (
        ...
        if isinstance(raw_selection_context, dict):
            ...
            if parent_session_id:
                from pathmind.services.session.organization import (
        ...
        if update_blocked:
            with contextlib.suppress(Exception):
                await self.store.transition_turn(
                    turn["id"],
                    "failed",
                    error="PathMind is preparing an update; try again after it reconnects",
                    failure_code="rejected",
                )
            raise RuntimeError("PathMind is preparing an update; try again after it reconnects")
        ...
        if mastery_binding is not None and mastery_lease_managed:
            ...
            if persisted_turn is None or persisted_turn.get("status") != "running":
                ...
                from pathmind.learning.storage import LearningStore
