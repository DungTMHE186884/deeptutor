...
from pathmind.reading.references import resolve_reading_sources
...
async def collect_prior_image_attachments(
    ...
    *,
    ...
    """
    if limit is None:
        from pathmind.services.config import get_prior_image_reinject_limit
...
# (see ``pathmind/api/routers/imports.py``).
...
def _load_partner_group_reference(
    ...
    *,
    ...
    try:
        from pathmind.services.partner_groups import get_partner_group_manager
...
async def _load_question_entry(store: SessionStoreProtocol, entry_id: int) -> tuple[str, str]:
    ...
    from pathmind.services.session.turn_runtime import _format_question_bank_entry
