...
from pathmind.services.llm.utils import clean_thinking_tags
...
if TYPE_CHECKING:
    from pathmind.runtime.coordination import TurnLease
...
def _reading_action_context(
    ...
    try:
        from pathmind.reading import ReadingStore, material_summary, search_material
...
def _reading_references(value: Any) -> list[dict[str, Any]]:
    ...
    from pathmind.reading.references import normalize_reading_references

    return normalize_reading_references(value)


def _llm_selection_dict(value: Any) -> dict[str, str] | None:
    from pathmind.services.model_selection import LLMSelection
