...
(:class:`pathmind.services.sandbox.backends.RunnerSidecarBackend`).
...
Wire contract (must match ``RunnerSidecarBackend``):
  ...
  ``POST /exec``   -> request/response JSON described by the dataclasses in
                      :mod:`pathmind.services.sandbox.spec`. Request::
...
Mounts note:
  ...
  root (``PATHMIND_RUNNER_ALLOWED_WORKDIRS``).
"""
...
_ALLOWED_WORKDIR_ROOTS = [
    ...
    for root in os.environ.get(
        "PATHMIND_RUNNER_ALLOWED_WORKDIRS",
        "/workspace/outputs",
    ).split(":")
    if root
]
