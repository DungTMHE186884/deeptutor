...
from pathmind.services.sandbox.spec import (
    ExecRequest,
    ExecResult,
    IsolationLevel,
)
...
def _active_python_prefix(*, inherit: bool) -> tuple[Path | None, bool]:
    """Return PathMind's Python prefix, including a base Conda environment.
...
class RestrictedSubprocessBackend(SandboxBackend):
    ...
    def __init__(
        ...
        *,
        ...
        Desktop installs run PathMind from a virtual environment but can
        inherit a host ``PATH`` whose first Python belongs to MSYS, Conda, or a
        system install.  Prepending the environment that launched PathMind
