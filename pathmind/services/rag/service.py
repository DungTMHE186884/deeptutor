...
class RAGService:
    ...
    PathMind's authoritative KB config, with metadata as a legacy fallback.
    """
    ...
    def _resolve_provider(self, kb_name: Optional[str]) -> str:
        """Pick the provider for ``kb_name`` from PathMind's binding."""
    ...
    def _capture_raw_logs(self, event_sink):
        ...
        from pathmind.logging import ProcessLogEvent, capture_process_logs
        from pathmind.logging.process_stream import ProcessLogHandler
        ...
        def should_skip_noisy_retrieve_log(event: ProcessLogEvent) -> bool:
            ...
            if logger_name == "nano-vectordb" and (
                ...
                return True
            if (
                logger_name.startswith("pathmind.services.embedding.")
                and (
