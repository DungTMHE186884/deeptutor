...
PathMind first (see ``ingestion``) so GraphRAG only ever sees ``.txt`` input.
...
from pathmind.runtime.home import get_runtime_data_root
from pathmind.services.rag.index_versioning import (
    resolve_storage_dir_for_read,
    resolve_storage_dir_for_rebuild,
)
from pathmind.services.rag.kb_paths import resolve_kb_dir
...
class GraphRagPipeline:
    ...
    def _ensure_available(self) -> None:
        if not gr_config.is_graphrag_available():
            raise gr_config.GraphRagNotAvailableError(
                ...
                "`pip install 'pathmind[graphrag]'` to use GraphRAG knowledge bases."
            )
    ...
    async def _preflight_settings(self, settings: dict[str, Any]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="pathmind-graphrag-preflight-") as temp_dir:
    ...
    async def add_documents(self, kb_name: str, file_paths: List[str], **kwargs) -> bool:
        ...
        from pathmind.services.rag.embedding_binding import bound_graph_storage_root
...
def _context_to_sources(context_data: dict[str, Any]) -> list[dict[str, Any]]:
    """Map GraphRAG context records into PathMind's source-citation shape."""
