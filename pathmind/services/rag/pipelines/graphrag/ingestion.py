...
(its optional ``markitdown`` reader is just a converter). So PathMind owns the
"document → text" step and hands GraphRAG ready ``.txt`` files. We deliberately
reuse PathMind's existing extraction primitives here so multimodal/parsing
stays a PathMind-side concern (matching how the LlamaIndex pipeline already
