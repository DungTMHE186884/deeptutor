"""Bridge PathMind's runtime config into a GraphRAG ``settings.yaml``.
...
PathMind's already-resolved LLM + embedding runtime config and let
...
* Completion calls use PathMind's registered GraphRAG compatibility adapter,
...
from pathmind.services.config.embedding_endpoint import canonical_embedding_provider_name
from pathmind.services.embedding.request_options import should_send_embedding_dimensions
...
# appends the ``/embeddings`` operation path to ``api_base``. PathMind stores a
...
class GraphRagNotConfiguredError(RuntimeError):
    """Raised when PathMind's LLM / embedding config can't back GraphRAG."""
...
def is_graphrag_available() -> bool:
    ...
    extra: ``pip install 'pathmind[graphrag]'``. Until it is installed the
...
def graphrag_embedding_api_base(binding: str | None, endpoint: str | None) -> str:
    """Translate a PathMind embedding endpoint into GraphRAG ``api_base``.

    PathMind's public embedding contract stores and calls the complete
    ...
    Args:
        binding: Active PathMind embedding binding.
...
def ensure_graphrag_embedding_transport(
    ...
    # Gemini can use either PathMind's native ``batchEmbedContents`` adapter
...
def query_config_from_settings() -> GraphRagQueryConfig:
    ...
    try:
        from pathmind.services.config import load_graphrag_settings
...
def build_settings(*, llm_cfg: Any = None, embedding_cfg: Any = None) -> dict[str, Any]:
    """Assemble the GraphRAG ``settings.yaml`` payload from PathMind config.

    ``llm_cfg`` / ``embedding_cfg`` are injectable for tests; in production they
    are resolved from PathMind's catalog. Raises
    :class:`GraphRagNotConfiguredError` if either side has no usable model.
    """
    if llm_cfg is None:
        from pathmind.services.config import resolve_llm_runtime_config
    ...
    return {
        ...
        # Plain-text input: PathMind's ingestion writes parsed ``.txt`` files
        ...
        # GraphRAG/LanceDB defaults to 3072 dimensions; PathMind must stamp the
