"""Configuration helpers for PathMind's LlamaIndex RAG pipeline."""
...
def should_show_progress() -> bool:
    ...
    PathMind runs as a server that stream is a pipe whose read end (the
    launcher's relay thread) can close mid-indexing, and the next tqdm write
    then raises :class:`BrokenPipeError`, killing document indexing. PathMind
...
def retrieval_config_from_env() -> RetrievalConfig:
    ...
    return RetrievalConfig(
        profile=normalize_retrieval_profile(
            os.getenv("PATHMIND_RAG_RETRIEVAL_PROFILE") or os.getenv("RAG_RETRIEVAL_PROFILE")
        )
    )
...
def _load_runtime_settings() -> dict:
    ...
    from pathmind.services.config import load_llamaindex_settings
