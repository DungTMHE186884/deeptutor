"""Bridge PathMind runtime configuration into the LightRAG 1.5 native SDK.

This module is the decoupling seam: it exposes availability + mode helpers and
builds the three adapters LightRAG needs from PathMind's already-resolved LLM,
...
Decoupling notes:
* ``llm_model_func`` / ``vision_model_func`` wrap PathMind's unified model
  callables and DROP LightRAG's internal kwargs (``hashing_kv``,
  ``keyword_extraction``, …) so they never leak into ``factory.complete``.
* ``embedding_func`` reuses PathMind's embedding client, wrapped in LightRAG's
...
if TYPE_CHECKING:
    from pathmind.multi_user.models import CurrentUser
...
class LightRagNotConfiguredError(RuntimeError):
    """Raised when PathMind's LLM / embedding config can't back LightRAG."""
...
def _retry_classification(exc: Exception) -> tuple[bool, str]:
    ...
    from pathmind.services.llm.request_compat import is_transient_transport_error
...
def _retry_delay_seconds(exc: Exception, scheduled_delay: float) -> float:
    ...
    from pathmind.services.llm.error_mapping import retry_after_seconds
...
def is_lightrag_available() -> bool:
    ...
    Opt-in extra: ``pip install 'pathmind[rag-lightrag]'``. Until installed the
...
def query_kwargs_from_settings() -> dict:
    ...
    try:
        from pathmind.services.config import load_lightrag_settings
...
def resolve_lightrag_query_llm_config():
    ...
    from pathmind.services.model_selection.runtime import resolve_llm_config_for_selection
...
def build_llm_model_func(
    *,
    ...
    """Wrap PathMind's unified LLM callable for LightRAG.
    ...
    if llm_config is None:
        ...
        from pathmind.services.llm.client import build_model_func_for_config
...
def build_vision_model_func(
    *,
    ...
    """Map rc2 ``image_inputs`` to PathMind's vision callable."""
    if llm_config is None:
        from pathmind.services.llm import get_llm_client
...
def vision_model_available() -> bool:
    """Return whether the active PathMind model is explicitly vision-capable."""
    try:
        from pathmind.services.llm import get_llm_client
...
def build_embedding_func(*, io_bridge: OwnerLoopBridge | None = None):
    """Wrap PathMind's embedding client in LightRAG's ``EmbeddingFunc``."""
    from lightrag.utils import EmbeddingFunc

    from pathmind.services.embedding import get_embedding_client, get_embedding_config
