...
PARSER_ENGINE = "pathmind"


class LightRagContractError(RuntimeError):
    """Raised when the pinned SDK returns a shape PathMind cannot trust."""
...
def installed_version() -> str:
    try:
        ...
        raise LightRagContractError(
            "LightRAG is not installed. Install `pathmind[rag-lightrag]`."
        ) from exc
...
def _require_exact_version() -> None:
    ...
    if current != LIGHTRAG_VERSION:
        raise LightRagContractError(
            f"PathMind requires {LIGHTRAG_DISTRIBUTION}=={LIGHTRAG_VERSION}; found {current}"
        )
...
def _controlled_class():
    ...
    class PathMindLightRAG(LightRAG):
        def _resolve_source_file_for_parser(
            ...
            *,
            ...
            if parser_engine not in (None, PARSER_ENGINE):
                raise IngressError(
                    f"Unsupported parser engine for PathMind ingress: {parser_engine}"
                )
...
def _register_parser() -> None:
    ...
    register_parser(
        ParserSpec(
            ...
            impl="pathmind.services.rag.pipelines.lightrag.parser:PathMindParser",
            suffixes=_SUFFIXES,
            queue_group="native",
        )
    )
...
def workspace_for(working_dir: Path) -> str:
    ...
    return f"pathmind_{hashlib.sha256(identity).hexdigest()[:16]}"
