...
ENTRY_POINT_GROUP = "pathmind.reading_extensions"
...
_BUILTIN_EXTENSIONS: dict[str, str] = {
    "read_aloud": "pathmind.reading.read_aloud:ReadAloudExtension",
    "guided_learning": "pathmind.reading.study_guidance:StudyGuidanceExtension",
    "vocabulary": "pathmind.reading.vocabulary:VocabularyExtension",
    "quiz": "pathmind.reading.quiz:ReadingQuizExtension",
    "translation": "pathmind.reading.translation:TranslationExtension",
}
...
def _builtin_extensions() -> list[ReadingExtension]:
    ...
    out: list[ReadingExtension] = []
    for name, target in _BUILTIN_EXTENSIONS.items():
        module_name, _, attr = target.partition(":")
        try:
            value = _coerce(name, getattr(importlib.import_module(module_name), attr))
        except Exception:
...
class ReadingExtensionRegistry:
    def __init__(self, extensions: list[ReadingExtension] | None = None) -> None:
        ...
        if extensions is None:
            # Source checkouts run from PYTHONPATH without installed package
            # metadata, so the entry-point group can be empty; the built-in
            # extensions are always available.
            for row in _builtin_extensions():
                self._extensions.setdefault(row.manifest.id, row)
