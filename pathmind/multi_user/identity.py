...
from pathmind.services.file_io import atomic_write_text
...
# process FastAPI deployments (the ``pathmind start`` launcher) are fully covered;
...
def _canonical_record(
    ...
    *,
    ...
    record.update(_profile_fields(value))
    return record


# Registration profile fields. Accounts created before detailed registration
# simply lack them (no email → email verification never applies).
def _profile_fields(value: dict[str, Any]) -> dict[str, Any]:
    out: dict[str, Any] = {}
    full_name = str(value.get("full_name") or "").strip()
    if full_name:
        out["full_name"] = full_name[:120]
    email = str(value.get("email") or "").strip().lower()
    if email:
        out["email"] = email[:255]
        out["email_verified"] = bool(value.get("email_verified", False))
        if value.get("email_verified_at"):
            out["email_verified_at"] = str(value["email_verified_at"])
    return out
...
def _env_bootstrap_admin() -> tuple[str, str]:
    ...
    :mod:`pathmind.services.auth` is imported lazily because it imports this
    ...
    """
    try:
        from pathmind.services import auth as auth_service
...
def save_user(
    ...
    """Create or update an account. ``profile`` (full_name / email /
    email_verified) overrides the stored profile fields when given."""
    USERS_FILE.parent.mkdir(parents=True, exist_ok=True)
    ...
    with _USERS_WRITE_LOCK:
        ...
        record.update(_profile_fields({**existing, **(profile or {})}))
...
def find_user_by_email(email: str) -> tuple[str, dict[str, Any]] | None:
    """Case-insensitive lookup of the account that registered ``email``."""
    needle = (email or "").strip().lower()
    if not needle:
        return None
    for username, record in load_users().items():
        if str(record.get("email") or "").lower() == needle or username.lower() == needle:
            return username, record
    return None


def update_profile(username: str, **fields: Any) -> dict[str, Any] | None:
    """Merge profile fields (full_name, email, email_verified, …) into an account."""
    with _USERS_WRITE_LOCK:
        users = load_users()
        ...
        for key in ("full_name", "email", "email_verified", "email_verified_at"):
            record.pop(key, None)
        record.update(_profile_fields(merged))
        _write_users(users)
        return record
...
def set_disabled(username: str, disabled: bool) -> dict[str, Any] | None:
    """Lock (``disabled=True``) or unlock an account without touching its data."""
    with _USERS_WRITE_LOCK:
        users = load_users()
        record = users.get(username)
        if record is None:
