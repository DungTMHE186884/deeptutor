...
(:mod:`pathmind.services.partners.interaction`), so two people talking to the
...
def social_partner_ids(user_id: str) -> set[str]:
    """Partners a friend shared with the user, or bound to a room they are in.

    Both only ever grant *use*. Read from the billing/social database; any
    failure there simply means "no social access" rather than an error.
    """
    if not user_id:
        return set()
    try:
        ...
        with get_db_session() as db:
            shared = {
                row.partner_id
                for row in db.query(PartnerShare.partner_id).filter(
                    PartnerShare.shared_with_id == user_id
                )
            }
            rooms = {
                row.partner_id
                for row in db.query(SharedRoom.partner_id)
                .join(RoomMember, RoomMember.room_id == SharedRoom.id)
                .filter(
                    RoomMember.user_id == user_id,
                    SharedRoom.is_active.is_(True),
                    SharedRoom.partner_id.isnot(None),
                )
            }
        return {pid for pid in shared | rooms if pid}
    except Exception:
        return set()


def assigned_partner_ids(user_id: str | None = None) -> set[str]:
    """Partner ids the user may use without owning them.

    Admin assignments (grants) plus partners shared by friends or bound to a
    study room the user belongs to (see ``social_partner_ids``).
    """
    ...
    return granted | social_partner_ids(uid)
