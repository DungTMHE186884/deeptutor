...
from pathmind.partners.channels.weixin_qr import ILINK_APP_ID
from pathmind.partners.config.schema import DeliveryOverrides
from pathmind.partners.helpers import safe_filename, split_message
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not self._token:
            ...
            # PathMind (or auto-starting a Partner) must never take over the
    ...
    async def _send_text(
        ...
        client_id = f"pathmind-{uuid.uuid4().hex[:12]}"
