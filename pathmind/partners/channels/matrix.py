...
try:
    ...
    raise ImportError(
        "Matrix dependencies not installed. Run: pip install pathmind[matrix]"
    ) from e
...
from pathmind.partners.config.schema import DeliveryOverrides
    ...
    async def start(self) -> None:
        ...
        if self.config.e2ee_enabled and not MATRIX_E2EE_AVAILABLE:
            raise ImportError(
                ...
                "`pathmind[matrix-e2e]` or `requirements/matrix-e2e.txt`, and make "
    ...
    def _decrypt_media_bytes(self, event: MatrixMediaEvent, ciphertext: bytes) -> bytes | None:
        if decrypt_attachment is None:
            logger.warning(
                ...
                "Install pathmind[matrix-e2e] to decrypt encrypted media."
            )
            return None
        key_obj, hashes, iv = (
