...
def discover_channel_names() -> list[str]:
    ...
    import pathmind.partners.channels as pkg
...
def load_channel_class(module_name: str) -> type[BaseChannel]:
    ...
    from pathmind.partners.channels.base import BaseChannel as _Base

    mod = importlib.import_module(f"pathmind.partners.channels.{module_name}")
    for attr in dir(mod):
        obj = getattr(mod, attr)
        if isinstance(obj, type) and issubclass(obj, _Base) and obj is not _Base:
            return obj
    raise NotAChannelModule(f"pathmind.partners.channels.{module_name} defines no channel")
...
def discover_plugins() -> dict[str, type[BaseChannel]]:
    ...
    for ep in entry_points(group="pathmind.partners.channels"):
        try:
            cls = ep.load()
            plugins[ep.name] = cls
