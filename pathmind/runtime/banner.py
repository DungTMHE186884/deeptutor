"""Branded banner + localized labels for ``pathmind start`` / ``pathmind init``.
...
from pathmind.__version__ import __version__
...
LABELS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "init.next_step": "Run `pathmind start` to launch PathMind.",
        "init.choice": "Choice",
        ...
        "start.detached_started": "PathMind is starting in the background (launcher PID {pid}).",
        "start.detached_already_running": (
            "A detached PathMind launcher is already running (PID {pid}). Log: {log}"
        ),
        "start.detached_log": "Log: {path}",
        "start.detached_stop_hint": 'Stop it with `pathmind stop --home "{home}"`.',
        ...
        "start.frontend_restart_failed": (
            ...
            "Stop PID {pid} and run `pathmind start` again."
        ),
        ...
        "stop.not_running": "No detached PathMind launcher is running.",
        "stop.requested": "Requested a graceful stop from launcher PID {pid}.",
        "stop.complete": "PathMind stopped.",
        ...
        "start.port_in_use": (
            "PathMind cannot start because port(s) already in use: {ports}. "
    ...
    "zh": {
        ...
        "init.next_step": "运行 `pathmind start` 启动 PathMind。",
        "init.choice": "请选择",
        ...
        "start.detached_started": "PathMind 正在后台启动（launcher PID {pid}）。",
        "start.detached_already_running": (
            "已有后台 PathMind launcher 正在运行（PID {pid}）。日志：{log}"
        ),
        "start.detached_log": "日志：{path}",
        "start.detached_stop_hint": '运行 `pathmind stop --home "{home}"` 停止。',
        ...
        "start.frontend_restart_failed": (
            ...
            "请先停止 PID {pid},然后重新运行 `pathmind start`。"
        ),
        ...
        "stop.not_running": "当前没有后台 PathMind launcher 在运行。",
        "stop.requested": "已请求 launcher PID {pid} 正常停止。",
        "stop.complete": "PathMind 已停止。",
...
def render_banner(language: str | None, *, mode_key: str | None = None) -> Panel:
    ...
    return Panel(
        Align.left(body),
        title="[bold bright_cyan]PathMind[/]",
