...
fixed by PathMind code and are never accepted from an API request.
"""
...
_WORKER_MODULE = "pathmind.runtime.worker_process"


def _worker_limit() -> int:
    try:
        return max(1, int(os.environ.get("PATHMIND_ISOLATED_WORKERS", "2") or "2"))
    except ValueError:
        return 2
...
def run_in_isolated_process_sync(
    ...
    *args: Any,
    ...
    with _WORKER_SLOTS:
        with tempfile.TemporaryDirectory(prefix="pathmind-worker-") as temp_dir:
