"""Runtime home resolution for installed and source PathMind runs."""
...
PATHMIND_HOME_ENV = "PATHMIND_HOME"
...
def validate_runtime_home(home: str | Path) -> None:
    ...
    if resolved in invalid_homes:
        raise ValueError(
            f"Invalid PathMind runtime home: {resolved}. Runtime data must live under "
            f"{PACKAGE_ROOT / 'data'} without nesting. Start PathMind with "
            f"PATHMIND_HOME={PACKAGE_ROOT} from {PACKAGE_ROOT}."
        )
...
def get_runtime_home(home: str | Path | None = None) -> Path:
    ...
    2. ``PATHMIND_HOME`` environment variable.
    ...
    raw = home if home is not None else os.getenv(PATHMIND_HOME_ENV)
