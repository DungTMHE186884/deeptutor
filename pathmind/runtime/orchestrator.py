...
from pathmind.capabilities.protocol import AGENT_OUTPUT, EVENT_METADATA
from pathmind.core.context import UnifiedContext
from pathmind.core.stream import StreamEvent, StreamEventType
from pathmind.events.event_bus import Event, EventType, get_event_bus
...
from pathmind.runtime.stream_bus import StreamBus, register_bus, unregister_bus
...
class ChatOrchestrator:
    ...
    async def handle(self, context: UnifiedContext) -> AsyncIterator[StreamEvent]:
        ...
        try:
            from pathmind.services.rag.pipelines.pageindex import (
        ...
        async def _run() -> None:
            ...
            from pathmind.services.llm.metrics import TurnUsage, current_usage

            current_user = get_current_user()

            # Enforce plan: allowed model + enough credits for this capability.
            try:
                from pathmind.database.connection import get_db_session
                from pathmind.services.quota_service import check_turn_allowed

                overrides = context.config_overrides or {}
                selection = overrides.get("llm_selection") or {}
                from pathmind.services.quota_service import model_name_for_selection

                # llm_selection usually carries {profile_id, model_id}: resolve
                # it so the plan/credit check prices the model actually used.
                requested_model = (
                    overrides.get("model")
                    or overrides.get("model_name")
                    or model_name_for_selection(selection)
                    or ""
                )
                with get_db_session() as db:
                    allowed, reason = check_turn_allowed(
                        current_user.id,
                        db,
                ...
                if not allowed:
                    ...
                    await bus.emit(
                        StreamEvent(
                            type=StreamEventType.DONE,
                            source="orchestrator",
                            metadata={"status": "failed", "error_code": "quota_exceeded"},
                        )
                    )
                    await bus.close()
                    if _turn_id:
                    ...
                    return
            except Exception as quota_check_exc:
                # Fail-open so a DB hiccup never blocks learning, but make it visible.
                logger.warning("Orchestrator quota check skipped: %s", quota_check_exc)
            ...
            try:
                ...
                summary_data = turn_usage.summary() if turn_usage.calls else {}
                await bus.emit(
                    StreamEvent(
                        type=StreamEventType.DONE,
                        source=cap_name,
                        metadata={
                            "status": status,
                            **terminal_error_metadata,
                            **({"usage_summary": summary_data} if summary_data else {}),
                        },
                    )
                )

                # Credits are charged per LLM call (services/llm/metrics.py).
