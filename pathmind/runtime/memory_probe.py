"""Resident-memory snapshot of the running PathMind process tree.

The counterpart to :mod:`pathmind.runtime.memory_reclaim`: that module *acts*
...
Two things make "how much memory does PathMind use" more than one ``getrusage``
call:

* The backend is only one of the processes. ``pathmind.runtime.launcher``
  ...
  ``uvicorn pathmind.api.main:app`` run would climb into the developer's shell
...
# find the root of the PathMind process tree it belongs to.
SUPERVISOR_PID_ENV = "PATHMIND_SUPERVISOR_PID"
...
class ProcessMemory:
    """One process in the PathMind tree."""
...
def _classify(pid: int, name: str, cmdline: str, root_pid: int | None = None) -> str:
    ...
    the supervisor runs as a bare ``pathmind`` console script with nothing in
    ...
    if "next" in lowered or "node" in lowered:
        return "web"
    if "uvicorn" in lowered or "pathmind.api" in lowered:
        return "backend"
...
def _scan_proc() -> tuple[list[ProcessMemory], bool]:
    """psutil-free fallback for Linux — the platform PathMind ships in Docker on."""
