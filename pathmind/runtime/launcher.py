"""Local Web launcher for the installed PathMind app."""
...
from pathmind.runtime.banner import labels_for, print_banner, resolve_language
from pathmind.runtime.home import (
    PATHMIND_HOME_ENV,
    PACKAGE_ROOT,
    get_runtime_home,
    validate_runtime_home,
)
from pathmind.runtime.memory_probe import SUPERVISOR_PID_ENV
from pathmind.runtime.process import is_process_alive
from pathmind.services.app_update import LAUNCHER_PID_ENV

BACKEND_READY_TIMEOUT_ENV = "PATHMIND_BACKEND_READY_TIMEOUT"
FRONTEND_READY_TIMEOUT_ENV = "PATHMIND_FRONTEND_READY_TIMEOUT"
...
SOURCE_PRODUCTION_DIST_DIR = ".next-pathmind"
SOURCE_BUILD_MARKER = ".pathmind-build.json"
...
DETACHED_WORKER_ENV = "PATHMIND_DETACHED_WORKER"
DETACHED_TOKEN_ENV = "PATHMIND_DETACHED_TOKEN"
...
def _reset_runtime_singletons() -> None:
    """Make a just-selected PATHMIND_HOME visible to path/config singletons."""
    try:
        from pathmind.services.path_service import PathService

        PathService.reset_instance()
    except Exception:
        pass
    try:
        from pathmind.services.config.runtime_settings import RuntimeSettingsService
...
def _packaged_web_dir() -> Path | None:
    try:
        ...
        return None
    path = Path(pathmind_web.__file__).resolve().parent
...
def _copy_packaged_web_if_needed(
    ...
    *,
    ...
    marker = cache / ".pathmind-web-runtime.json"
...
def _ensure_web_dependencies(source: Path, npm: str) -> None:
    ...
    if result.returncode != 0:
        raise SystemExit(
            ...
            "Fix the error above, then retry `pathmind start`."
        )
...
def _source_production_env(*, api_base: str, auth_enabled: bool) -> dict[str, str]:
    ...
    env = os.environ.copy()
    env["PATHMIND_NEXT_DIST_DIR"] = SOURCE_PRODUCTION_DIST_DIR
...
def _source_build_fingerprint(source: Path, env: dict[str, str]) -> str:
    ...
    version_file = PACKAGE_ROOT / "pathmind" / "__version__.py"
...
def _ensure_source_production_build(
    ...
    *,
    ...
    Long-running ``pathmind start`` processes must not use ``next dev``: its
...
def _resolve_frontend(
    ...
    *,
    ...
    if packaged is not None:
        if not node:
            raise SystemExit("Node.js 20+ is required to run the packaged PathMind Web app.")
    ...
    raise SystemExit(
        "PathMind Web assets are not installed. Install the full app with `pip install -U pathmind`, "
...
def _launch_detached(
    ...
    *,
    ...
    env = os.environ.copy()
    env[PATHMIND_HOME_ENV] = str(runtime_home.resolve())
...
def stop(home: str | Path | None = None, *, timeout: float = 15) -> bool:
    """Request a graceful stop from a detached PathMind launcher."""
...
def _handoff_pending_update(
    ...
    *,
    ...
    from pathmind.services.app_update import (
        UpdateJobStore,
...
def _complete_restarted_update(runtime_home: Path) -> bool:
    ...
    from pathmind.services.app_update import UpdateJobStore, update_store_root
...
def start(
    ...
    *,
    ...
    os.environ[PATHMIND_HOME_ENV] = str(runtime_home)
    ...
    common_env[PATHMIND_HOME_ENV] = str(runtime_home)
    ...
    common_env["PATHMIND_API_BASE_URL"] = backend_url
    common_env["PATHMIND_AUTH_ENABLED"] = "true" if auth_enabled else "false"
    common_env["PYTHONUNBUFFERED"] = "1"
    common_env["PYTHONIOENCODING"] = "utf-8:replace"
    # Anchor for pathmind.runtime.memory_probe: the backend and the frontend
    # are siblings under this process, so only the supervisor's pid identifies
    # the tree that is "PathMind". Without it the probe can only measure the
    # backend itself.
    ...
    # ``PATHMIND_NEXT_DIST_DIR`` is a build-only override.  In particular it
    ...
    # clean the live ``.next-pathmind`` tree out from under the standalone
    ...
    common_env.pop("PATHMIND_NEXT_DIST_DIR", None)

    backend_cmd = [
        sys.executable,
        "-m",
        "uvicorn",
        "pathmind.api.main:app",
        "--host",
        "0.0.0.0",
        "--port",
        str(backend_port),
        ...
        # middleware (pathmind/api/main.py) surfaces only non-200s, so routine
