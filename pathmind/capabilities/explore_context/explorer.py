...
* **Fallback** (:meth:`_single_pass`) — for providers without native tool
  ...
  single pass, modelled on :class:`~pathmind.agents.notebook.analysis_agent`.
"""
...
from pathmind.runtime.agentic.messages import assistant_message, assistant_message_with_tool_calls
...
from pathmind.services.llm import clean_thinking_tags, get_llm_config, get_token_limit_kwargs
from pathmind.services.llm import stream as llm_stream
from pathmind.services.llm.capabilities import threads_session_id
from pathmind.services.session.provider_response_state import normalize_provider_response_state
...
class ContextExplorer:
    ...
    async def _call_llm(
        ...
        if threads_session_id(self.binding):
            kwargs["pathmind_session_id"] = f"{session_id}:explore"
        if tool_schemas:
