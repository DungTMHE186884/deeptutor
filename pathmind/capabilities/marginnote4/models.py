...
These types describe MN4 objects as they land in PathMind after sync.
...
* ``raw`` preserves the original MN4 JSON for fields PathMind does not model
...
class MarginNoteObject:
    ...
    Attributes:
        ...
        synced_at:  ISO-8601 timestamp when PathMind received this version.
...
@dataclass(slots=True)
class PairedDevice:
    """A MarginNote 4 device registered with this PathMind instance.
...
class LearningEvent:
    ...
    Captures a review or study interaction so PathMind can track mastery
