...
mastery engine (:mod:`pathmind.learning`).
...
from pathmind.capabilities.mastery.choices import (
    canonical_labels,
    has_option_bodies,
    labelled_options,
    option_label_intent,
...
from pathmind.capabilities.mastery.mode import (
    MODES,
    REVIEW,
    admission_error,
    enforced_mode,
...
from pathmind.learning.policy import (
    QUALITATIVE_TYPES,
    display_mastery,
...
from pathmind.learning.question_card import (
    QUESTION_CARD_KEY,
    attempt_number,
    build_grade_result,
    build_question_card,
...
if TYPE_CHECKING:
    from pathmind.learning.models import LearningProgress
...
async def _resolve_pending_choice(
    ...
    if not has_option_bodies(options):
        try:
            from pathmind.services.session import get_sqlite_session_store
...
async def _sync_mastery_attempt_to_question_bank(
    *,
    ...
    if not session_id:
        return
    from pathmind.learning.assessment import (
        AssessmentRecord,
...
def _wrong_mode_result(tool_name: str, kwargs: dict[str, Any]) -> ToolResult | None:
    ...
    :mod:`pathmind.capabilities.mastery.mode`. The learner-facing guarantee is
...
class MasteryGradeTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        if interaction is not None and interaction.status == InteractionStatus.ANSWERED:
            ...
            from pathmind.learning.pending import is_readable_choice_answer
...
class MasteryModeTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        from pathmind.capabilities.mastery.binding import remember_mode_on_session
...
class MasterySwitchTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from pathmind.capabilities.mastery.binding import (
            PathBindingError,
            rebind_active_path,
        )
...
def _normalized_module_tree(
    ...
    PathMind runs on whatever model the learner brings, and smaller ones
