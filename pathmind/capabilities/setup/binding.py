...
def setup_gaps() -> tuple[SetupGap, ...]:
    ...
    """
    from pathmind.services.config.settings_spec import setting_specs
...
# configuring PathMind itself.
_DIRECT_PATTERN = re.compile(
    r"(帮我配置|帮我设置|自己配置|配置一下\s*pathmind|设置一下\s*pathmind|初始化配置)"
    r"|(\bconfigure\s+pathmind\b|\bset\s?up\s+pathmind\b|\bsetup\s+wizard\b)",
    re.IGNORECASE,
)
