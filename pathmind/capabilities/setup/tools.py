"""The four tools through which PathMind configures itself.

They are deliberately generic: each one walks
:mod:`pathmind.services.config.settings_spec` rather than knowing anything
...
``apply_setting``
    Commit one row, after the checks in :mod:`pathmind.capabilities.setup.apply`.
...
class InspectSetupTool(BaseTool):
    ...
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            name="inspect_setup",
            description=(
                "Read PathMind's own configuration: every setting you can "
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from pathmind.capabilities.setup.access import can_write
        from pathmind.capabilities.setup.binding import setup_gaps
        from pathmind.capabilities.setup.jobs import available_jobs
        from pathmind.services.config.settings_spec import specs_for_area
...
class ApplySettingTool(BaseTool):
    ...
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            name="apply_setting",
            description=(
                "Change one PathMind setting. The value must be one of the "
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from pathmind.capabilities.setup.apply import apply_setting
    ...
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            ...
            description=(
                ...
                "entered directly into PathMind. Call it, tell the user what "
...
class RunSetupJobTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from pathmind.capabilities.setup.access import can_write
        from pathmind.capabilities.setup.jobs import run_job
