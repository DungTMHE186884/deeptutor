...
*Personal* rows are the ones :class:`~pathmind.services.path_service.PathService`
...
:func:`pathmind.api.routers.settings._require_settings_admin` already guards —
...
(:func:`~pathmind.multi_user.paths.get_owner_path_service`), so honouring a
configuration request from an IM-facing companion would mean letting whoever is
chatting with it reconfigure the owner's PathMind — including, if the owner is
...
from pathmind.services.config.settings_spec import Scope
    ...
    if _is_partner_turn():
        return AccessDecision(
            ...
            reason=(
                ...
                "The owner can make this change in PathMind directly."
            ),
        )
    if scope == "personal":
