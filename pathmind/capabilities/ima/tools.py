...
(see :class:`~pathmind.capabilities.ima.capability.ImaCapability`). They cover
...
from pathmind.capabilities.ima.binding import ImaBinding, resolve_client, select_binding
...
# this module is imported while ``pathmind.tools.builtin`` builds the global tool
# table, and reaching into ``pathmind.services.rag`` at import time would close
...
def _note_sort(name: Any) -> int:
    ...
    from pathmind.services.rag.pipelines.ima.notes import (
        SORT_BY_CREATED,
        SORT_BY_TITLE,
        SORT_BY_UPDATED,
    )
...
class _ImaTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from pathmind.services.rag.pipelines.ima.config import ImaNotConfiguredError
        from pathmind.services.rag.pipelines.ima.envelope import (
            ImaAPIError,
            ImaAuthError,
            ImaRateLimitError,
        )
...
class ImaReadTool(_ImaTool):
    ...
    async def _run(self, client, binding, kwargs):
        from pathmind.services.rag.pipelines.ima import media as media_ops
        from pathmind.services.rag.pipelines.ima import sources as source_policy
