...
def _concat_l3() -> str:
    ...
    """
    from pathmind.services.memory import get_memory_store, paths
...
def _resolve_partner_id() -> str | None:
    ...
    from pathmind.services.partners.scope import PARTNER_USER_PREFIX
...
class PartnerReadTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from pathmind.multi_user.paths import get_admin_path_service, get_current_path_service
        from pathmind.services.memory import memory_path_service_override
        from pathmind.services.partners.interaction import get_partner_turn_context
...
class PartnerMemorizeTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from pathmind.multi_user.paths import get_current_path_service
        from pathmind.services.memory import get_memory_store, memory_path_service_override
...
class PartnerSearchTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from pathmind.partners.config.paths import get_partner_sessions_dir
        from pathmind.services.partners.interaction import get_partner_turn_context
