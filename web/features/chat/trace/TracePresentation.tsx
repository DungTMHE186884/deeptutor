...
function getTraceHeader(
  ...
  if (
    ...
    // top-level status row carries the verbose "PathMind Reflecting…"
...
function detectStreamingMode(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (event.type === "content" && callKind === "llm_final_response") {
      ...
      // still open; keep the top-level title on "PathMind Exploring…" until
      // the bus moves on.
      if (event.stage === "exploring") return "exploring";
...
export function StreamingStatus({
  ...
  const name = agentName?.trim() || "PathMind";
  ...
  else if (mode === "responded") modeLabel = t("PathMind responded.");
 ...
 * reasoning trace once PathMind stops working and starts (or has finished)
 * its answer.
 *
 ...
 * ("PathMind Exploring… · 8s", settling to "PathMind responded. · 10s")
 * with the exploring trace nested directly beneath it.
 *
 * The trace is expanded by default while PathMind is still reasoning /
